...
\documentclass{NSF}
\ifdraft\else\iflater\pagestyle{empty}\fi\fi% Reset page style to 'empty'
...
\vspace*{2in}
\title{%
% BCP: Checked boilerplate 10/1/2023
SHF: HCC: Medium:
%
Property-Based Testing for the People}

\todo{
  Next round of TODOs from October 6, 2023 meeting:
  \begin{enumerate}
    \item Take a tightening pass over Section 3 [assigned to Andrew, but
    Benjamin already did quite a bit of tightening; another look would
    be good, but less critical now.
    ]
    \item Revisions to Section 2 [Benjamin done]
    \item Check to see if any of the details we remove from Section 2
    should be forward-ported to other sections to strengthen them with
    supporting empirical evidence [Benjamin: I think not critical]
    \item Clean up the “Diffusion” section: update the title, rewrite
      the part about research software  [Andrew done]
    \item Try to work in the idea of “usable PBT” at several (more) points in
    the grant application.
    \item Introduce the idea of “usable PBT” as follows: our ICSE
    study found out that PBT is useful, though not necessarily
    usable. [already done]
    \item Emphasize transformational nature of the project. Maybe by
    explicitly writing in the Introduction that our ideas will be
    transformational, focusing on how we reenvision how PBT tools can
    be designed in a way that affords unprecedented levels of
    visibility into what the tools are doing, and control over their
    behavior [everybody] [Andrew added draft paragraphs to intro. We should revise it, and then propagate our emphasis of transformational aspects to the appropriate section headers.]
    \item Etna: Benjamin mentioned that there may be additional
    interesting things to do in the benchmarking space  [Benjamin and Harry]
    \item A little more thought to PL stuff? Related to your point
    just above about reflectives framing, Benjamin and I talked a bit
    about whether we there is enough stuff for a more
    theoretically-minded PL student to work on. We didn’t have time to
    come to a conclusion.  [Benjamin and Harry]
  \end{enumerate}
}

\todo{
  \begin{itemize}
  \item Carefully read the CFP and make notes (Benjamin and Andrew done)
  \item Redo BPC in light of departmental plan? (Andrew) [first pass done;
    feedback desired before further changes; Benjamin: Looks pretty good.]
  \item Redo the management plan to fit Medium specification (Benjamin)
  \item Add some LLM motivation to section 1 (Harry)
  \item Get a letter from Dolan (he's
...
\includepdf[pages=-]{assets/CCB-letter.pdf}
