...
\todo{
  ...
  \begin{enumerate}
    \item Take a tightening pass over Section 3 [assigned to Andrew]
    \item Revisions to Section 2 [whoever does it first]
    \item Check to see if any of the details we remove from Section 2 should be forward-ported to other sections to strengthen them with supporting empirical evidence
    \item Clean up the “Diffusion” section: update the title, rewrite the part about research software
    \item Try to work in the idea of “usable PBT” at several points in the grant application.
    \item Introduce the idea of “usable PBT” as follows: our ICSE study found out that PBT is useful, though not necessarily usable.
    \item Emphasize transformational nature of the project. Maybe by explicitly writing in the Introduction that our ideas will be transformational, focusing on how we reenvision how PBT tools can be designed in a way that affords unprecedented levels of visibility into what the tools are doing, and control over their behavior
    \item Etna: Benjamin mentioned that there may be additional interesting things to do in the benchmarking space
    \item A little more thought to PL stuff? Related to your point just above about reflectives framing, Benjamin and I talked a bit about whether we there is enough stuff for a more theoretically-minded PL student to work on. We didn’t have time to come to a conclusion.
  \end{enumerate}
}
...
\todo{
  ...
  \item Redo BPC in light of departmental plan? (Andrew) [first pass done;
    feedback desired before further changes]
