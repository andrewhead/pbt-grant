...
\amh{[nit pick] I would rather we do not name the system we
are going to build in this proposal, because it suggests
that there is \emph{one} solution to the problem of
evaluating input distributions, when in reality, there are
many. I would rather present this section as a collection of
suitable interaction techniques which, many of which are
novel in their own right, and which would work even better
in concert.}
...
generator successfully produces an interesting distribution
of values. \amh{I would instead say that the task is to
understand where generators fall short of producing useful
input data distributions so that a developer
can improve them} Many
...
inputs that the generator covers. \amh{Make the problem
clearer. As I understand, the problems are that (1) a
generator may not generate the kinds of inputs that should
be tested at all, and (2) even if it does, it might do so so
slowly that the test driver will take too long to find
important failures.} Tools like this would help developers catch
...
concrete example \amh{Mention that this example comes from
one of the informants in our study},
consider a list of log events with type:
\begin{lstlisting}
  type log = {id : int; payload : string} list
\end{lstlisting}
\amh{Briefly describe the kinds of questions that the
developer would want to ask about the distribution to make
it more concrete why simple, one-size-fits-all data displays
will not be enough. Soime of the questions that come to mind
for me are: do the logs look about long enough? Do they have a
realistic distribution of the kinds of messages that would
appear in the actual logs? Is the test driver wasting time
repeatedly generating empty lists? Is there realistic
representation of dependencies within those lists (e.g.,
does a ``received'' event always follow a ``sent'' event)?}
...
needs to be efficient to invoke.

\begin{wrapfigure}{r}{0.6\textwidth}
  \centering
  \includegraphics[width=0.6\textwidth]{assets/gen-vis.pdf}
  \caption{A mockup of \genvis. (1) A {\em code lens} that the developer clicks
  to start \genvis. (2) High-level statistics about the generated values. (3)
...
literature.
\amh{nix the program synthesis, which I think is no longer
relevant. We are unlikely to make
contributions in visualization recommendation because it is
a vast literature, so let's not mention that. Instead, I
would highlight that this work explores how a novel
combination of tried-and-true features can support the task
of understanding the quality of input data distributions.
This includes (1) live, realtime displays of generated
values; (2) the ability to drill-down from aggregate
displays to individual values; (3) extensibility with
lightweight hooks for customizing aggregate data displays
and previews of individual points; (4) live feedback on what
code is covered by the generated inputs.}
...
would consider simple functions like \lstinline{length}, field accessors like
\lstinline{head}, \lstinline{id}, and \lstinline{payload}, and even aggregators
like \lstinline{maxBy} and \lstinline{avgBy}. Then \genvis{} will then use
...
dimensional feature). \amh{Do we think most functions will
meaningful filter functions like these for their input data?
If so, that is really important to mention here.} If there are features that the user notices should be
...
With the features extracted, \genvis{} will then select various charts and
...
\genvis{} will also have a way for users to specify which visualizations are
...
at example values.  \genvis{} will therefore also present the user with
structured representations of some generated values that can be used spot-check
the generated data. We are not yet settled on the specifics, since the
visualizations used for individual values may need to depend on the domain, but
we have some ideas. For some kinds of structures, it may be useful to present
the user with DOT graphs~\cite{ellson_graphviz_2002} that represent data types;
such graphs could be generated via meta-programming and rendered for the user.
Another option is to simply present the user with a JSON representation of the
data that they can explore via mouse interactions (e.g., clicking to expand or
collapse sections) or even via tools like \lstinline{jq}~\cite{dolan_jq_nodate}.
...
features. We will evaluate \genvis{} in a small user study (around 15
participants), hoping to learn which features of the tool are most useful, and
how the design may be refined into a tool that real
developers can use. \amh{Probably instead of mentioning the
usability study here, we should factor this out into an
``evaluation plan'' that covers all of the tools in this
section.} Once we
are happy with the design, we will publish the tool as an official extension,
maintain it as an open source project \amh{I am not sure we
want to commit to this; this sounds like a lot of work
especially if there are a lot of experimental, novel
features}, and publicize it to the testing community.

\SUBSECTION{Tuning Input Data Distributions}{sec:tuning}{1}{3}

\todo{Draft this out (HG: I can do this if you want, @AMH)}
...
\SUBSECTION{Adding PBT to 1210}{sec:1210}{2}{4}
