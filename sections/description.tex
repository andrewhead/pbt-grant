...
challenging new research questions based on developer
concerns\bcp{phrase in terms of usability}.
...
state of the art in PL+HCI: PI Head recently co-founded
a new HCI group at the University of Pennsylvania, while PI Pierce has
...
\item Our ongoing study is just the first step in laying a
\emph{foundation} for HCI-informed research on PBT.  We will confirm
its findings and further explore the challenges PBT faces in industry
with three additional studies---two\iflater\bcp{check!}\fi{} survey
studies and one observation study of PBT users {\em in situ}
(\sectionref{sec:foundation}).
  \item We will address challenges in property \emph{specification} by
  developing tools that \bcp{ugh}
  ...
  (\sectionref{sec:spec}).
  \item We will improve and simplify test-case \emph{generation} with new
  ...
  test input mutation, and example-based generator tuning. \bcp{Can we
  make that more usability-oriented?}
  (\sectionref{sec:gen}).
  \item We will close gaps in \emph{validation} of testing effectiveness by providing
  ...
  (\sectionref{sec:val}).
  \item We will create resources for PBT \emph{education}, including
  materials for adding PBT to an undergraduate data structures course,
  advice to developers on how to identify situations where properties
  are readily available, and a tool for helping naive users articulate
  properties (\sectionref{sec:ed}).
\end{itemize}
...
Jane Street is among the world's largest
...
\proposecut{There is a danger that findings from a study at a single
  firm with a homogeneous OCaml-based testing ecosystem may not
  generalize outside of this ecosystem. Part of our proposed work is
  to carry out further user studies targeting a broader and more
  diverse community (\sectionref{sec:foundation}).  However, Jane
  Street itself is quite diverse in the sorts of software it builds,
  including real-time trading, quantitative algorithms, networked
  systems, and hardware description code~\cite{signalsandthreads}, and
  our study participants come with a wide variety of experiences and
  requirements.}

% \bcp{Probably cut this, but make sure the HATRA study is promimently
%   cited somewhere.}
% % BCP: Done!
...
% at the SPLASH HATRA workshop, one of the key venues for
% emerging work on human aspects of formal methods tools and type
% systems~\cite{ref:goldstein2022some}.

As of December 2022, the full complement of 30 interviews and a
preliminary round of analysis have been completed; full-scale analysis
will begin in 2023.
...
For this reason, \evaltheme{Evaluation of Effectiveness} often featured in our
...
Education projects are discussed in \sectionref{sec:ed}.
\iflater
\bcp{Why don't we have a separate color / paragraph for education themes??}
\hg{I'd argue that we don't have ``education themes''---education is important
around all three sections. I don't know if any JS themes are education specific}
\fi
...
One finding that has surprised us from the Jane Street study is that it is {\em
...
\begin{figure}[h]
  ...
  BST invariant.\bcp{wrap it?}}\label{fig:mutation}
\end{figure}
...
\todo{Add references to related work on debugging,
including FireCrystal~\cite{ref:oney2009firecrystal},
Rehearse~\cite{ref:brandt2010rehearse},
Gamut~\cite{ref:hohman2019gamut},
Chameleon~\cite{ref:hohman2020understanding},
Lux~\cite{ref:lee2021lux},
Burg's Interactive Record/Replay
project~\cite{ref:burg2013record},
Examplore~\cite{ref:glassman2018visualizing}.
Reference what level of liveness we seek to support from
Tanimoto's work~\cite{ref:tanimoto1990viva}. There are examples of live environments for
functional programming like the Hazel
editor~\cite{ref:omar2019live} and Light
Table~\cite{tool:lighttable}. OmniCode~\cite{ref:kang2017omnicode} visualizes the distributions of values that
variables take on over a program's execution.}
...
\amh{Continue the rewrite from here, integrating additional
references, calling attention to these features:
(2) the ability to drill-down from aggregate displays to individual values; (4) live feedback on what code is covered by the
generated inputs.}
...
\amh{Add references to bidirectional editors including
Sketch-n-Sketch~\cite{ref:hempel2018deuce},
mage~\cite{ref:kery2020mage}, active
code completion~\cite{ref:omar2012active},
livelits~\cite{ref:omar2021filling}.}
...
\amh{Let's describe this tool as another tool in our
comprehensive PBT toolkit, rather than an extension to the
visualization tool. If reviewers don't like the
visualization, they might still like this. And I think it better
represents the fullness of the proposal when we describe
this as an independent yet synergistic tool.}
...
For more complex interactions, and in cases where filtering causes high discard
...
generators (introduced in \sectionref{sec:gen}) do. Recall that reflective
...
\amh{Add references to Titus' slow fixes
project~\cite{ref:barik2016quick},
CodeScoop~\cite{ref:head2018interactive}, Emerson's work on visual extract-method
refactorings~\cite{ref:murphyhill2008refactoring}, and
drag-n-drop refactoring~\cite{ref:lee2013draganddrop}, and test case
generation (e.g., RANDOOP), Daka et al.'s work on generated
readable test cases~\cite{ref:daka2015modeling}.}
