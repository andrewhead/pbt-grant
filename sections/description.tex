...
The first task is to figure out what kind of error this is.
Is it a parsing error? Is it an off-by-one error? You can
determine this from the input if it is of manageable size.

Interactive shrinking? Have someone edit the input, and then
ask for more inputs like the manually shrunk input. A more
structural interface to editing the input may be easier than
using a console. Can you do tuning of the generator to the
shape of a hyper-specific input?

Rapid, incremental, interactive shrinking of inputs. The
main feedback is just a check or an "ex" determining whether
