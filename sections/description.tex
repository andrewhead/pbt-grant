...
automating model-based testing (\sectionref{sec:spec}).
...
(\sectionref{sec:val}).
  \item We will propose a program\bcp{blah}\hg{What's blah about it? I'm not
  sure I know we're optimizing for}\bcp{``propose a program'' could
  mean almost anything---doesn't say that we're actually going to
  build curricular materials} for PBT \emph{education} including
...
%   by multiple people, or just P7?  Or are we extrapolating from other things
%   people said, about abstraction generally?)}\hg{I think properties over logs
%   has a bigger potential for impact, RE enabling new specifiction
%   opportunities}%
% BCP: Fine
\bcp{Was the log issue mentioned
   by multiple people, or just P7?  Or are we extrapolating from other things
   people said, about abstraction generally?  If we can mention more
   participants, the support seems much more powerful.}
\hg{Just P7, but lots of people in the pilot talked about spaghetti code too, so
it's fair to extrapolate. Also FWIW P7 did something sort of like this, just in
a very ad-hoc way with no LTL}\bcp{Can we say something to suggest we
heard similar thigngs from others?}
%
...
One Jane Street developer (\participant{7}) described their experience testing a
...
effectiveness.\hg{BCP or AMH, feel free to edit. I was just trying to make it
more exciting and mention usability}
...
inputs (\sectionref{sec:failures}), assess whether their generators are
generating sufficient, appropriate inputs
(\sectionref{sec:evaluating_distributions}) and whether those inputs
sufficiently exercise their code (\sectionref{sec:tuning}), and migrate failed
property-based tests into regression tests (\sectionref{sec:counter}).  These
...
\hg{I think all of this can go}
\proposecut{ First, it becomes a task in and of itself to understand individual
inputs. This because the inputs are not written by the developer, but rather
generated automatically, and furthermore, they can be of unbounded structural
complexity.  Second, a developer needs to assess the quality of their tests. The
quality of a property-based test depends in part on the extent to which it
covers execution pathways through the code, though it also depends on the
quality of the inputs.  Namely, are the inputs complex enough, and distributed
in such a way that they are likely to trigger practically important bugs? Third,
developers need to decide what to do with failed test cases, and may need to
spend time migrating failed test cases into regression suites. Finally,
developers need to make difficult decisions around how much computation time to
budget for exercising the property-based tests.
...
%
\bcp{Harry and I both persistently have trouble understanding what the
title means.  Can we find a pithier phrase?  Also, the section seems
much too long -- the most important bits are the first paragraph and
the last sentence.}\hg{How's this?}\bcp{Works for me.}
%
...
\end{ganttchart}
  \caption{Plan of work.
   \bcp{squeeze the header lines to the left}
...
amore. \bcp{I guess we need to write one?  And if so we should warn
  the business office that it's coming -- I'll do that now.}
...
\bcp{This needs more text from me.  ``...a summary of the results of the completed work, including accomplishments, supported by the award. The results must be separately described under two distinct headings: Intellectual Merit and Broader Impacts;''}
...
\todo{Unfunded collaborations: Any substantial collaboration with
