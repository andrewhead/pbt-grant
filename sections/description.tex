...
\amh{Can we do anything in this section to better highlight specific, concrete tasks
the lead student would work on? John is worried that in Section 4.1, there are
no concrete tasks for the lead student to work on, due to its focus on broad and
...
by some natural metric on their type, not on the random choices that
...
\amh{We can better motivate this choice by indicating (1) it has support for
OCaml and Haskell; and (2) according to this survey
\url{https://survey.stackoverflow.co/2022/#technology} it is the most
widely-used IDE.}
...
\amh{John comment: can we say something about the ``marketing campaign'' we
would have to undertake to get this used? Maybe adoption at Jane Street could
give us a place to start. Then we might need visibility in the developer
community with talks and tutorials at developer conferences, and PBT virtual
events. The PIs would likely need to be heavily involved. Maybe we need to refer
to this in the management plan. We might need to lean heavily on Jane Street or
other partners to help us with this.}
...
%
\amh{John thought that this should come at least a little bit after the
``Generalizability of Jane Street findings''}
\todo{Transition}
