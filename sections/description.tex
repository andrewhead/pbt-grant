...
\bcp{I crunched this to one paragraph.  But is it readable?  Should
  the headings be boldfaced instead of italic?}%
...
for the user to write down---they are simply example program inputs---but if
we are instead asking the user to write down
sequences of {\em
  choices} that lead to these seeds being {\em generated}\bcp{remind
  the reader why we are doing that}, then
it becomes much more
error-prone (and tedious) to produce seeds by hand\bcp{why not just
  say infeasible?}.  This is one excellent use for a
...
values. Reflective
generators can help with this by (1) reflecting on the
choices\bcp{I find this phrase persistently confusing: the RG is
  reflecting on [thinking about] the input value and {\em producing}
  the choices that lead to it.} that lead to a particular
value, (2) mutate those choices, and (3) run the generator forward with the new
...
that Blackwell described.)}\hg{Whatever you think is best}\bcp{Andrew,
can you add this bit?}
%
...
the help of real users.  We will recruit a small group of users, teach
...
\hg{Need a sentence about a masters course we'll also add PBT to (masters version of 1210)}
