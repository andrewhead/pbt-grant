...
\iflater\todo{Double-check that we've mentioned all relevant
  ``hgih-level'' related work here.}\fi
...
\proposecut{The interviews have focused on tasks that developers would like to
accomplish with PBT tools and their recollections of what it was like to
work with PBT tools to accomplish these tasks.}\bcp{Maybe that sentence
can go?  (What else would such interviews focus on?)}
...
\proposecut{The study format was semi-structured interviews exploring developers'
...
\proptheme{Model-Based Testing}. \proposecut{Model-based testing is not so much a scenario
in which to apply PBT as it is a technique for using it, but}
participants like\bcp{including? (but if there were multiple ones, why
  not list them all?)}
...
testing, it is difficult to know when ``enough is enough.'' In
...
stop.\bcp{That comparison doesnt really hit home: with PBT, too, the
  one test that you didn't run might be the one that catches the bug.} This tension leads to a variety of approaches to
...
thousand examples, but some run it for much longer. Interestingly, the opposite
was true about a close cousin of PBT, fuzz testing. Participants
like\bcp{again, if there are more why not just list them?}
...
strategies like {\em mutation
  testing}~\cite{papadakis_mutation_2018}\proposecut{; we plan
...
\hg{What else does this need? Does it work?}\bcp{I think it's good!
  If you want, we could save a tiny bit of space by grouping all the
