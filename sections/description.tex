...
\proptheme{specification}
discussed in the motivation section. One major finding of the study is that
widening the on-ramp to PBT requires pushing developers toward high-leverage
use-cases for PBT. We discuss our plans to teach developers about high-leverage
scenarios in \sectionref{sec:whento}; in this section, we discuss opportunities
to expand and improve the space of scenarios where PBT shines.  In
\sectionref{sec:outpurprop} we improve PBT's applicability to poorly abstracted
code, and in \sectionref{sec:automating} we describe tools that automate 
automatically in a specific high-leverage scenario: model-based testing.
Together, these projects will greatly improve the on-ramp to PBT and clarify its
place in the software industry.
