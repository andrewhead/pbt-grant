...
tests\proposecut{ (including, often, property-based tests)}, has 9.6K stars~\cite{PytestGitHub}---i.e., Python's
...
high-leverage ways to extend PBT's reach\proposecut{ by
...
improving open-source PBT projects\bcplater{as John pointed out, we
  need to be careful with that one...}, building a
...
\iflater\bcp{Needs a paragraph talking about why fuzzing is also not
  the answer.  ``Conversely, if good coverage of the space of
  potential inputs is important, why not use a standard fuzzer?''
  (Answer: Fuzzers work best with trivial ``does not crash''
  properties and with whole systems, not smaller components...)}\fi
...
\subsectionstar{Usability Challenges} The final product from
...
Participant~9~(\participant{9}), PBT is particularly easy to apply
when
...
developers write programs like this\bcplater{Did anyone really guess
  100\%??}, where properties are easy to find and
...
The findings from the Jane Street user study\iflater\bcp{We call
