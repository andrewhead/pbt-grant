...
right here. Like ``The goal of this project is to re-envision the practice of PBT
...
though.}\bcp{I don't object to some kind of exciting statement of
goals here, but I'm having trouble coming up with wording that works
well (``Envision the practice of PBT for 2023'' only makes any sense to me
after thinking about it for a while.)  So I'm going to leave it for later.}
...
one conference publication on the industry study mentioned above~\cite{Goldstein2024ICSE},
two workshop
...
\begin{wrapfigure}{r}{.45\textwidth}
  \vspace*{-2ex}
  \centering
  \includegraphics[width=.45\textwidth]{assets/overview.pdf}
...
themes (summarized in the figure on the right):
%
...
easier to {\em assemble} code to carry out a given task, but even harder to
...
\lstinline{is_bst} checks that its argument is a BST---i.e., whether each node's label is greater
...
The dominant approach to generating inputs and the one we use is {\em random}
...
\sectionref{sec:reflectivefuzzing}).  In general, fuzzers need to run
...
to test both whole systems and smaller subcomponents.  Both are useful, but
they are applied in different ways, at different points, to achieve
different goals.

With all the advantages of PBT, one might hope to find it on every
software developer's toolbelt.  But using PBT effectively
can be also challenging, as we shall see next.
...
the ICSE technical paper track~\cite{Goldstein2024ICSE}.  In this
...
(\sectionref{sec:foundation}) are intended to help complete the
...
One research opportunity that is highlighted by this
...
of their tools' ability to \gentheme{derive
  generators} from data type specifications. At the same time,
...
\amh{How is this rewrite?}
A second opportunity is to help developers to define properties, or
\emph{specifications}, describing intended program behavior. In another
preliminary interview study we conducted~\cite{goldstein_problems_2022},
some developers reported difficulty \emph{imagining
properites} to test. Tools for PBT could alleviate this challenge by
helping developers ideate properties. Beyond this, tools could help developers
implement those properties. Developers at Jane Street told us about
myriad \emph{high-leverage scenarios} in which they regularly turned to PBT---such
as testing round-tripping behavior, differential testing, mathematical
properties, and data structure invariants. In some of these scenarios (e.g.,
round-tripping, data structure invariants), properties can be written concisely and naturally with
existing tooling. Others (e.g., differential testing) required
substantial amounts of boilerplate. PBT tools should allow developers
to define all of these kinds of properties quickly and crisply.
Furthermore, developers who are beginners to PBT should be
made aware of these high-leverage scenarios in which they should use PBT.

\if 0
\bcp{The first and fourth sentences here seem contradictory.  Maybe
  the whole paragraph needs a rethink.}%
A second opportunity is to equip developers
with more powerful means to define properties, i.e., \thread{specifications}.  PBT is often described as
...
\thread{interaction} mechanisms to visualize their tests'
effectiveness
...
Finally, our study underscored the fact that \thread{diffusion} of PBT
...
to improve \edutheme{open-source PBT frameworks}.
...
The industry study described in \sectionref{sec:motivation} paints a clear
...
{\em non}-users of PBT. For this survey, we will sample more broadly,
aiming to identify developers
...
% {\em too} many studies...}%
%
The surveys described in \sectionref{sec:survey} will provide a {\em broad}
understanding of
the modern experience of PBT. We will also conduct
observation studies to develop a \emph{deep} understanding of particular tasks
related to PBT---e.g.,
language features for implementing
generators (\sectionref{sec:gen}) or interfaces for evaluating and tuning data
distributions (\sectionref{sec:val}). In such settings it is critical
...
\publicationtargets{Findings from these studies will provide material
  for the ``formative study'' sections of the technical publications
...
Many of the usability challenges highlighted by our prior interview study
are related to test input
\gentheme{generation}.
Indeed, generators were regularly cited as one of the most challenging
aspects of PBT.  The wealth of papers on generation in the PBT
literature tells a similar story.

Our overarching goal in the tasks described in this section is to
improve the {\em controllability} of
generators.
...
help to connect PBT and fuzzing (\sectionref{sec:reflectivefuzzing}) and
design methods for making reflective generators easier to
build~\sectionref{sec:reflectivepeople}. Finally, we introduce benchmarking
frameworks by which the PBT community can evaluate improvements in generation (\sectionref{sec:benchmarks}).
Success for these enabling technologies will be measured both by the success of
tools built on them (see \sectionref{sec:val} and \sectionref{sec:diffusion})
and in terms of publications at top-tier venues.

John Hughes, one of the QuickCheck creators and a longtime
collaborator of PI Pierce, plans to work with us on the projects in
this section; a letter of collaboration from Prof.{} Hughes appears in
the supplemental documents.

\subsectionstar{Context: Why Random Generation is Hard}{}{}
As discussed in \sectionref{sec:orientation}, PBT relies heavily on {random
generators}.  Checking a property for many randomly generated inputs
increases confidence that it holds, but this confidence is well
placed only if the inputs actually
exercise a wide variety of program behaviors.
Unfortunately,
many properties of interest have {\em preconditions}
(a.k.a.\relax{} {validity conditions} or {input constraints}).  For example,
the \verb|prop_insert_correct| function in \sectionref{sec:orientation} succeeds trivially if
the input tree does not satisfy \verb|is_bst|.  Such preconditions
come up frequently in
practice; they are problematic because they can be
...
%
And even generators that produce valid
inputs can under-perform in other ways---for example, by
generating many very similar inputs while ignoring large parts of
the input space.
% \amh{Is the preconditions problem the \emph{main} reason that random generation
% is hard? Or even one of the main reasons? The current writing doesn't sell how
% central this is, and so might run the risk of feeling like it is fixating on an
% unimportant detail.}
% \bcp{Is it certainly one of the main reasons.  I tweaked the wording a
% little.}

Approaches to these challenges
fall on a spectrum from automatic to manual. More automatic approaches use
...
generators, often written in a special-purpose Domain-Specific Language.
...
representations like grammar-based generators.  Moreover, monadic generators are
syntactically constrained in a way that isolates the probabilistic code and
prevents usage errors like propagating random seeds incorrectly.
...
reflective generators~\cite{goldstein2023reflecting} won a
distinguished paper award at ICFP 2023.

The defining feature of reflective generators is that they can be run
{\em backward}.
Whereas running a generator in the usual way can be seen as parsing a
sequence of choices into a
value, running it backward takes that value and produces a
sequence of choices that
would generate it.
The mathematical machinery
involved is somewhat involved,%
\footnote{\normalsize For experts: Reflective generators are both
  monads and {\em partial profunctors},
...
that of familiar monadic generators.
...
Reflective generators have many potential uses in PBT.  First,
%
it is often helpful to start testing with
``realistic'' inputs that trigger common-case behaviors in the program;
one way to ensure this is to tune the generator so
it produces values that are similar to some user-supplied values that
are deemed
...
informed by the choices that we saw. The evaluation in~\cite{goldstein2023reflecting} shows that reflective
generators approximate the performance of the much-discussed ``inputs
from hell'' algorithm
...
also be used to systematically {\em enumerate choices} or make {\em dynamically guided
choices}.% \amh{It's not clear to me why this is important.}
...
%
The existing reflective generators paper only scratches the surface
when it comes to building and evaluating potential use-cases; there is
much more to do.
%
First, we believe more powerful approaches to example-based tuning are
within reach.  The
example-based tuning algorithm used in
that paper assumes that subsequent choices are
...
account. We propose modeling choices and their contexts via a neural
network\amh{do we have any examples of successfully using neural networks in
Pierce group projects in the past? If we do, citing them here can make it feel
like we have a good shot at doing it here.}\bcp{Nope, it's speculative!},
rather than a simple distribution map, compressing the data into a compact
representation that is easy to query.
This representation, if successful, should make the example-based tuning
procedure far more accurate, allowing it to much more faithfully replicate
important features of complex input sets.
\amh{If we stick with neural networks as our method for modeling, I think we
should move this idea second (after fuzzers), as it feels less fully
developed.}\bcp{Agreed, though reordering will take a bit of
work. Feel free to have a go.}
...
are demonstrably useful, but neither benefits from the years of expertise
...
reason, but also {\em reflective}.  The reasoning is that,
besides their general flexibility and broad set of
use-cases, reflectives should be useful for fuzzing because their backward
...
it is infeasible to ask the user of a Crowbar-like system to write
down the sequences of choices that
result in their seed values.
Fortunately, this is an excellent use for the
backward interpretation of a reflective generator. The user can write
down their seeds---either as values
in the program, or as text that can be parsed by the program's parser---and
ask the reflective generator to figure out the choices that lead to
those seeds.
...
\SUBSECTION{Reflective Generators for the People}{sec:reflectivepeople}{2}{2}{PBT
  Theory}{PhD 2}{Engineer}{Pierce}
%
Our goal is to see reflective generators incorporated in
real PBT frameworks.
%
However, it is unclear
how reflective generators can be
represented in languages with weaker type systems than Haskell's. In
Haskell, the type checker can automatically detect many
common errors in writing reflective generators, but its ability to do
so relies heavily on type-classes that
quantify over ``higher-kinded types'' (e.g., the {\tt Monad} type class). If
we hope to implement
reflective generators in frameworks like Python's Hypothesis library, we need to
design a programming discipline that makes building them equally safe and
intuitive. We plan to
to design
a form for reflective generators that is appropriate in languages like
Python, Java, and Rust, potentially informed by prior work on representing
monadic languages in imperative languages~\cite{brachthauser_representing_2021, delaat_pymonad_nodate}.

Another barrier to adoption is that a modicum of extra work is
...
the help of real users, informed the HCI
literature~\cite{ref:ko2015practical}.  Concretely, we will recruit a small group of users, teach
...
the users to identify points of friction and ensure a clear
understanding of their
impressions. With this information in hand, we will identify potential changes
to the reflective generator abstraction and test those changes with a
new group of users.  The
ultimate measure of success (again, evaluated through user studies)
is whether our abstraction for reflective generators is usable by non-expert PBT
practitioners.
...
We will assist
users in generalizing existing generators to reflective ones with
tools that automatically synthesize the annotations
required to make the generator reflective. We will experiment with both
...
We plan to collaborate in this effort with professor Hila Peleg at the
Technion, who has experience with both PBT and program synthesis. A
letter of collaboration appears in the supplemental
documents.
...
\publicationtargets{International Conference on Functional Programming (ICFP)}
...
%
Most papers
% \amh{citations may be expected with this phrasing}\bcp{I
%   guess we could grab a bunch from the Etna paper...}
in the existing PBT literature use small case studies to evaluate their tools,
...
to demonstrate the promise of an idea, but such evaluations
are hard to interpret from the perspective of a would-be user.
% We can do better \amh{at what?}, and we plan to when doing the work laid out in the previous
% two subsections.\amh{Kind of weird to have backward reference before we state
% the goal.}

In prior work, we have built a robust empirical evaluation
framework for
test input generation strategies, called Etna, providing infrastructure
for easily and extensibly running experiments to understand PBT testing
effectiveness.  By ``easily,'' we mean that Etna  takes
on the burden of
collecting data and analyzing the results.  Etna
helps users and developers evaluate a given tool based on
% (1) the
...
Coq), multiple frameworks (QuickCheck, QuickChick, SmallCheck, etc.), and
various workloads that we currently support, the
infrastructure is designed that users can easily add new
languages, frameworks, and workloads.

Some initial comparisons using Etna and a description of its
architecture appeared in an experience report at ICFP
2023~\cite{shi2023etna}, but there is much more to do. First, finding
pre-placed bugs is not the
only way to evaluate a testing tool. We plan to extend Etna with
ways of measuring and analyzing metrics like line coverage and branch
coverage, which could serve as additional evidence of an approach's
value. Second, we will extend Etna for evaluating PBT
tools in Python and OCaml. Bringing Etna to Python will
allow the developers of Hypothesis (including our collaborator Zac Hatfield-Dodds), to evaluate their testing tools in a controlled
environment. Likewise, Stephen Dolan, author and maintainer of
the Crowbar library in OCaml~\cite{dolan2017testing}, wants to run
experiments using Etna once we include support for OCaml.

Furthermore, we will codify a substantial library of case-studies and examples as
{\em benchmarks for PBT}.
% \amh{Isn't this what Etna already did?}
% Depends what you mean by "substantial" :-)
Similar suites of benchmarks already exist in the
fuzzing literature~\cite{hazimeh_magma_2021}, but these are not
organized around the particular challenges faced by PBT users. In particular,
few deal with the kinds of complex preconditions that PBT
...
well as other experiments by external PBT library developers}
...
involves applying PBT in {\em high-leverage scenarios}---instances where properties are
...
Jane Street,
it is quite common for developers to build (or already have at hand) a {\em
...
PBT~\cite{hughes_experiences_2016} and is closely related to ideas like
...
significant amounts of boilerplate code to define intermediate data
structures and ad-hoc test harnesses for comparing module outputs across a
series of function calls.

We have carried out some early experiments in automating this process in
{\Mica}, an experimental tool that automatically
writes properties based on simple interfaces for stateless modules
developed in a Penn student project advised by PI
Pierce~\cite{Ng2023MicaPoster}.
We propose to extend and generalize this work to address more
...
generates chains of \lstinline{S}-operations and checks that
...
% \amh{Do we have any evidence that achieving
% sufficient further improvement will be possible?}
%
We therefore plan to extend \Mica{} three dimensions: (1) to make it generate automatic
...
features, such as Java, Scala, and Rust.  Fourth and more speculatively, we will
investigate whether a similar automatic generation strategy can be
...
%
Developers in our formative study often described particular
testing scenarios as {\em not} being high-leverage for PBT because
their code was not well organized for applying
PBT.  PBT works best on components with clear boundaries; it does not
...
how to ameliorate such situations by writing properties over user-defined
...
\lstinline{Outer} took accepted simple inputs from the outside world
and used them to construct
...
PBT is difficult to apply, because the behaviors of the two components were too
closely entangled to test them separately.

We plan to develop a tool that allows developers to express properties that inspect
the state of multiple interacting software components. Going back to
the example of
\lstinline{Inner}, a solution to the developer's problem could be to drive the
test through \lstinline{Outer} but to phrase properties in terms
of the behavior of \lstinline{Inner} by allowing them to inspect a log
of the internal values that appear while it
...
\lstinline|@never@|, and \lstinline|@eventually@| to describe
how the internal state should evolve over
time; these are not available in conventional PBT
frameworks.

One might hope to simply check properties like this with inline
``assert'' statements, but this does not give enough expressive
power. On the one hand, assertions require writing nontrivial code to
save previous values for properties like (1) above, and would then
likely involve brittle macros or meta-programming to remove such
auxiliary code at release time. On the other hand, it is difficult to
see how to use assertions to check properties (2) or (3) at all,
because this requires that the assertions be cognizant of the ``end''
of a computation---information that may not be easily available.

Instead, we propose to develop a {\em Linear Temporal Log Testing
  Logic} (LTLTL) that
directly supports properties like the ones above. The temporal
operators in LTLTL are inspired by temporal logics such as standard
Linear Temporal Logic
(LTL). To use LTLTL, programmers will annotate their programs with lightweight
log statements that output messages like
\lstinline|processed = 10| and \lstinline|overflow = true|, then write
compact properties like the ones above to check temporal relationships
between the
logged messages. The result will be a simple scheme for checking quite complex
properties, placing no specific requirements on the architecture of
the code under test.

Temporal logics similar to LTLTL have been used for PBT in the
past~\cite{oconnor_quickstrom_2022}, but applying the ideas from that
work to our
...
in a program trace. For another, ordinary LTL expressions have been
found by some to be
difficult to use~\cite{greenman_little_2022}; we plan to design a
simplified set of temporal connectives that are more intuitive for users.
%
The outcome of this task will be a tool that compiles properties written
...
\SUBSECTION{A Programmer's Assistant for Specifications}{sec:ghostwriter}{3}{3}{HCI Practice}{PhD 2}{PhD 4}{Head}
%
...
how to help people know {\em when} they are in such a situation.
A preliminary study that we ran to prepare for the larger one described in
\sectionref{sec:motivation}~\cite{goldstein_problems_2022} suggests that
testers---beginner and experienced alike---sometimes face a kind of
writer's block in deciding what properties to test.

A variety of programming-assistant tools have recently come into wide
use for situations like this. Perhaps most familiar are
Copilot~\cite{tool:copilot}, an in-editor AI that suggests code
completions~\cite{ref:dohmke2022github}, and the GPT line of tools
(e.g.,~\cite{tool:chatgpt,ref:openai2023gpt4}), which in numerous
recent papers~\cite[etc.]{ref:bubeck2023sparks,
  ref:savelka2023thrilled} have shown their potential to turn informal
textual specifications of surprising complexity into working code.

One might be tempted to try using these tools to produce properties
directly, but we feel this is precisely the wrong approach. Test
suites for critical code are part of an application's {trusted
  computing base} and, as such, should be carefully and thoughtfully
chosen. Large language models encourage developers to cut and paste
rather without thinking, and they are known to be prone to
hallucination~\cite{ji2023survey}; it is a bad idea to use them to
directly write an application's tests.
%
...
%
We do, however, believe that LLMs have a significant role to play
...
Our envisioned assistant, dubbed {\em \GhostChat}, will
wrap existing property synthesis techniques---including Hypothesis's ``classic
GhostWriter''~\cite{ref:ghostwriter},
QuickSpec~\cite{ref:claessen2010quickspec}, and \Mica{} and LTLTL
(proposed in the prior two sections)---under APIs that allow them to
be invoked by an LLM programming assistant AI, similarly to other
recent programming library integrations for ChatGPT like the Wolfram
API.

The interaction between this hybrid assistant and a developer might go
...
When the user clicks on an action, the AI tries to determine the
kinds of properties that may apply for this module
and to recommend them in a list adjacent to the
...
arbitrary positions in the input sequence.'' This list may be enough
for developers
to overcome their writer's block. If it is not, they can further click
an action button to ``Look for Potential Automations.'' The
programming assistant will
then try to find appropriate use-cases for tools like {\Mica},
QuickSpec, and GhostWriter, and, if a use-case
exists, allow the programmer to run the corresponding tool automatically.
...
Under the hood, suggestions will be generated by a large language model
...
tools are added, use cases can either be added manually or scraped from
...
PI Pierce recently initiated a collaboration on programming using
large language models with Professor Chris Callison-Burch, an expert
on computational linguistics and LLMs who will work with us on this
...
Our final research thread focuses on developing powerful
\emph{modes of interactions} that afford developers new levels of
visibility and control in PBT. The main contributions will be novel techniques for evaluating testing effectiveness and improved mechanisms for shaping the distributions of generated inputs.
...
In this style of research, advances come in several forms: insights into task
...
implementations
with human users. \proposecut{Success will be measured by
reduction in
testing and debugging time and increased effectiveness\bcp{quantified
  or probed how?}.}
...
%
Our primary aim in this thread is to develop an integrated interactive
toolkit to expedite the creation of good property-based tests. We will
focus our efforts on a particular aspect in PBT where interaction
seems particularly suited to improving current processes: understanding and
shaping input data distributions.

The effectiveness of PBT depends on the quality of input
distributions, i.e., whether generated test cases are both sufficiently ``interesting'' and sufficiently diverse.
% \todo{Cite the UIST demo -- proof of concept for feedback.  We will do
But it is by no means easy for a developer to assess the quality of
an input distribution.
A first challenge is simply understanding the distribution along testing-critical
axes (e.g., heights of trees, fraction valid, etc.). Today, developers
are forced to infer the distribution along these axes by
drawing conclusions from the description of the generator or by creating custom
visualizations.
A second challenge is assessing whether the distribution is
``good''---sufficiently representative of real-world inputs, likely to exercise
bug-inducing paths in the software, etc.
A common strategy for developers is to assess input distributions by inspecting a
...
\begin{wrapfigure}{r}{0.58\textwidth}
  ...
  (4). Iteration is supported with live adjustment of
  generator code and refinement of filters used in the
  visualizations.}\label{fig:gen-vis} \vspace*{-1ex}
\end{wrapfigure}
%
...
realistic logs of records with timestamps
and event types.
%
...
We will develop Tyche
as an extension to Visual Studio Code, a modern editor, widely
adopted~\cite{noauthor_stack_nodate}, that supports every language
with mature PBT tooling. We plan for Tyche to serve as a platform
for developing not just the interactive features described here, but
also contributions
by other groups. A prototype of Tyche has been
accepted for the UIST demo session~\cite{ref:goldstein2023tyche}; a
mockup appears in Figure~\ref{fig:gen-vis}. The novel features that we
will develop in the proposed project are described below.

\subsectionstar{Understanding Input Distributions}{}{}
The first innovation will be the design of live displays of generated
...
provide this feature generically. What is unique in our domain is to provide
suitable representations of complex inputs that
will be easy to understand. \proposecut{Some general solutions might be to
...
pattern whereby a tool shows which lines of code are currently
executing~\cite{ref:brandt2010rehearse,
  ref:oney2009firecrystal, ref:burg2013record}, our tools will
colorize code to show
how {\em frequently} each line is executed while running tests\proposecut{, both in aggregate and
...
understanding distributions of complex input data. \proposecut{Success of this (and the
next) project will be determined by the novel interactions' ability to reduce
...
values can be challenging\proposecut{, requiring
significant trial and error with the source code definition of a
generator\amh{someone check this}}.
...
described in the previous section. The goal is to enable the kind of
...
includes plans for increasing the {\em diffusion} of PBT by developing the Tyche IDE as an
open-source project (\sectionref{sec:ide}), educating
...
We will advertise our findings, tools, and materials to potential user communities
through talks at industry conferences like \emph{Yow!}, Lambda Jam, Strangeloop, and
Lambda World.
...
our collaborators on the Hypothesis team (see Collaboration Plan)\proposecut{ to
...
leaders at Amazon AWS, who are interested in devoting engineering
...
public-facing, open-source project and creating smooth mechanisms for external collaboration.
%
...
have a tacit
understanding of a set of ``no-brainer'' situations where PBT is a natural
choice---where properties are easy to find and PBT provides
more thorough testing than other techniques---and
that they are happy to limit their use of PBT to such situations.
...
drive adoption among those less experienced with PBT.
...
%
Universities often act as force
multipliers to drive adoption of new
...
projects.
%
Our instructional materials will also be made available to
...
Broad impact is central to our research agenda, and many of the
...
the software industry within the time frame of the grant and beyond, driven
by more powerful and usable tools, driven principally by our open-source
tools (\sectionref{sec:ide}). Our goal is to
transition Tyche into a shared-ownership, open-source project
by the end of the project.
Success will be measured by the existence of a
significant user community outside of Penn using Tyche and
...
interested in putting engineering effort into developing Tyche and
...
\proposecut{An ancillary educational goal is to
...
(CACM).\amh{This is a great idea, though it feels like one extra thing on top of
an already full proposal. I think maybe we should cut this goal, or propose it
might be done following the conclusion of the project.}}
...
groups (2/4 of PI Head's current PhD students and 1/3 of PI
Pierce's are women).
...
NSF REU program that will involve 24 undergraduates (eight per
year for three+ years), selected specifically with an eye to diversity;
for the present project, we are requesting funds to add one more student per
...
to use Jane Street as a testbed for prototypes.  We are also
collaborating with the
Hypothesis developers, who are keenly
interested in our results, and we are in early
discussions with AWS about potential
...
available mutation testing framework for Python, called {\tt
  pytest-mutagen}~\cite{pytestmutagen}, and applying ideas from
