...
\bcpread{Para}%
The process of designing these properties is non-trivial, and is an example of
...
be correct. Luckily, prior work catalogs a number of different kinds of
properties that one might write~\cite{HowToSpecifyIt}, and we address challenges
with expressing properties in \sectionref{sec:spec}.
...
Libraries}\hg{BCP feel free to suggest a new name here---I changed
``communities'' to ``libraries'' but I'm still not attached to the
theme}\bcp{Tools?  But maybe needs a deeper rethink.}
to help our
work and ideas permeate the PBT world.

The overall agenda for this proposal, and many of the specific
tasks---are inspired by these motivating themes.
% \iflater
...
The ultimate findings from the Jane Street study\iflater\bcp{We call
  it a user study here, a need-finding study above, and I think a
  foundational study somewhere.  Let's rationalize this.}\hg{I propose we call
  it a ``study'' everywhere but the first time we introduce it (where we're more
  specific, in context, that it's a need-finding study). I propose we save
  ``user-study'' for tool evaluations and other things that are more
  traditionally user-studies}\fi{} should
give us a clear
...
Observations will allow us to evolve singular anecdotes from the Jane Street study
...
why it's a top-level task).}\hg{Any better?}\amh{I tried my hand at this section
as well. It is now probably too redundant, but might make the importance and
effort involved in the tasks a little more clear. We can probably go further.}
...
will be a new cognitive theory of PBT itself.  Theories of
programming have
...
\bcpread{Para}%
Like prior theories, a successful outcome is a theory that is simple, validated,
...
of effort.\hg{The last one is useful, but the other two seem too obvious. Can we
...
tooling (\sectionref{sec:reflective}). \bcpread{Rest of para}We explore how reflective generators can
...
plans to improve generator automation (\sectionref{sec:genauto}) and metrics
by which to evaluate generation improvements (\sectionref{sec:benchmarks}).
