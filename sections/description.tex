...
In the industry study, we found that a significant chunk of real-world PBT
focuses on {\em high-leverage scenarios}---instances where properties are
readily available and developers can jump right in to the process of finding
bugs. Thus, in this section we propose work to help developers write
specifications in the context of high leverage scenarios. First we propose tools
for supporting an existing high-leverage scenario, in particular {\em
model-based} or {\em differential} testing of classes and
modules~(\sectionref{sec:model-based}). Next we propose a technique for
inducting a new class of scenarios into the ``high-leverage'' category, with the
help of linear logic~(\sectionref{sec:ltl-logs}). Finally, we bring these
...
One surprising finding from our industry study was that, at least in this
environment, it is quite common for developers to build (or already have) a {\em
model implementation} of the code they are testing and want to test that the two
...
%
Since this situation is so common we want to make it as easy as possible for
developers.  We have already done early exploration on {\sc Mica}~\cn, a tool
...
continuing this work and broadening it to work in a wider range of languages.

{\sc Mica} can automatically build test suites that compare simple OCaml
modules. Given a module signature (\lstinline{S}) and two modules
(\lstinline{M1} and \lstinline{M2}) implementing that signature, {\sc Mica}
automatically generates chains of \lstinline{S}-operations and ensures that
\lstinline{M1} and \lstinline{M2} behave the same way when those operations are
invoked. This automates a process developers already do manually. It can
determine when a refactored module has subtly changed its behavior or when a
complex module disagrees with a simpler reference implementation.

So far, {\sc Mica} has captured an useful set of OCaml module features, but there
is still work to be done. The technique does not yet work well with stateful
modules, which operate by modifying some internal state, rather than by passing
module state between the functions in the signature. Furthermore, the tool
currently relies on information from the module's type signature to assemble
chains of operations, but we would like to extend this technique to work in more
...
significant swath of module testing situations, massively lowering the barrier
to entry for PBT.
...
A common reason developers described scenarios as {\em not} being high-leverage
for PBT was
that
...
Perhaps the most compelling observation about high-leverage PBT scenarios is
that they provide a sort of recipe that developers can use to decide when and
