...
%
(c) write random data generators for data structures with complex invariants,
%
...
save the specifics for the bullet points below.}
...
al.~\cite{chasins_pl_2021} describe that by pursuing a joint methodology 
involving
both PL and HCI achieves a ``sweet spot'' where need-finding techniques identify 
current pain points while leading to concrete tools that help programmers write 
safe, correct programs. We wholeheartedly agree, and adopt such a methodology as 
...
\item We establish a \emph{foundation} for HCI-informed research on PBT, 
...
(\sectionref{sec:foundation}).
  \item We will address usability challenges in property \emph{specification} by
  developing tools to \bcp{ugh:}
  automatically derive properties for the common use case of
  model-based testing, and we will facilitate specifying
  poorly abstracted code with a notation for writing and checking
  temporal properties over logged outputs (\sectionref{sec:spec}).
  \item We will improve and simplify test-case \emph{generation} with new
  domain-specific languages\bcp{??} for generators that enable novel
  ...
  make that more usability-oriented?  And mention the word ``reflective''?}
  (\sectionref{sec:gen}).
  \item We will improve \emph{validation} of testing
  ...
  (\sectionref{sec:val}). \bcp{Again, I want to lean into the idea of
    usability.}
...
\iflater\bcp{This needs a bit of focusing and toning down at some
  point.}\fi The projects we undertake will be motivated by
foundational need-finding research in the software industry,
including, but not limited to, the aforementioned study with Jane
Street.\bcp{What ``foundational need-finding research in the software
  industry'' do we have in mind besides the JS study?}\hg{1.1 and 1.2?} Some projects
will have a clear PL flavor, focusing on domain-specific languages and
type-based tools that will make PBT tools more powerful and flexible;
but we will motivate and evaluate them with techniques from HCI. Other
projects will focus on the HCI elements of PBT, building better
interfaces and experiences for developers; yet their implementation
will draw from our wealth of PL knowledge. In this way, we craft a
holistic research agenda around PBT that is sure to be both well
motivated and well founded.
...
not. The function \verb|prop_insert_correct| checks
...
satisfies the
binary search tree invariant (``\verb|is_bst t|''), then it should remain
a BST following the insertion of \verb|x| (``\verb|is_bst (insert x t)|)''.
...
will begin in 2023.  A paper describing our findings will be submitted
