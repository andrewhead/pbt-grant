...
up expressiveness: add some logging \proposecut{(printfs or whatever)\hg{``or
whatever'' reads as dismissive}} into the
...
not... ppxs are arbitrary code transformations}
...
process.  \bcp{But now that they're done, why is more research needed?}\hg{Is it
not compelling that developers continue to waste time on the same automatable
task? I thought that was a slam dunk...}
...
\hg{Same problem as the previous section---I'm working from hunches here.}
...
The PBT literature addresses these concerns to various degrees\bcp{but
not ``fully''?}, and existing tools
...
examples~\cite{soremekun2020inputs}.  \bcp{Why are developers not happy with
