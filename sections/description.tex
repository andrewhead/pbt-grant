...
{\Mica}~\cite{Ng2023MicaPoster}\amh{Did we develop this tool? If so, we should
say so explicitly}, an experimental tool that automatically
...
realistic modules and module-like language features.\amh{This paragraph probably
begins before the paragraph just before.}
...
representation of sets.\amh{Do we have any evidence that achieving
sufficient further improvement will be possible?}

\amh{Strange to have this paragraph split when both of these paragraphs
describe future changes. It makes the paragraphs feel like they are two
attempts at providing the same information.}
...
PBT\amh{simplify this sentence}.  PBT works best on components with clear boundaries and does not
...
test through \lstinline{Outer} but\amh{?} write properties about
...
way the internal state should evolve over time.\amh{I think the crucial thing to
mention here is that these kinds of operators are not available in typical PBT
frameworks.}
...
Instead, we propose {\em Linear Temporal Log Testing Logic} (LTLTL) \amh{which
we will develop?}, which
...
past~\cite{oconnor_quickstrom_2022}, but adapting\amh{why adapting? the way this
is described, it sounds like exactly what we are proposing.} that work to this
