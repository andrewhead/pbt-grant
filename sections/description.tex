...
%   work:
\hg{I think the Orientation section is too dense (and long) to make it clear
...
As with the \writeme{preliminary} study, the study with Jane Street is a
...
right specifications. Indeed, the \writeme{preliminary} study concluded just
...
\emph{Validation survey}. While our interview study was
revealed a number of issues with existing PBT tools, it can
be difficult to determine how broadly these issues are
experienced, and their severity, from interviews alone. Of
the usability issues identified in our interview study,
which ones most merit the attention of the research
community? To answer this question, we will conduct a survey
asking developers to report which of the issues they have
personally experienced, and to estimate the severity of the
issue when they encountered it. Respondents will also be
asked to elaborate on some of the most extreme obstacles
they have experienced, in order to collect more motivating
stories to guide future tool design.

The sample for our survey will be\ldots{}
...
properties were already available or quite easy to imagine. \bcp{I
  think readers are going to have trouble keeping the distinction
  between the ``preliminary study'' and the ``Jane Street study''
  (which is also preliminary, relative to this proposal) clear in
  their heads.  Could we just lump them together as ``our preliminary
  studies''?} This suggests that,
...
With this in mind, we plan to write a short\bcp{why short?} paper
...
participants of the preliminary study\bcp{same comment as above}) was
that code may not be abstracted
in a way that is conducive to PBT. Indeed, any kind of unit
testing\bcp{is PBT a kind of unit testing??} requires
...
abstraction boundaries\bcp{why?}. Fuzzers \todo{I think we need to
  intro fuzzers in the
beginning; I've mentioned them a bunch already}\bcp{Yes: in the
Orientation section, I guess?  This will be important for getting
non-PBT people on the same page as us.} get around these issues by
...
We hypothesize a solution that enables better PBT of messy
code\bcp{why only of messy code?} without giving
...
value of \lstinline{x} every time it is set\bcp{manually?}, and then check that the logged
...
could capture that too. This would be difficult to do with debug assertions\bcp{why?},
especially if performance is a concern\bcp{can't it just be turned
  off in production if performance is a concern?}.
...
\bcp{Identifying a challenge is fine, but can we be clearer about the
  contribution that our proposed tool will make?}
...
target is often very large, and the best generators explore the whole\bcp{?!?}
space as
...
terms\bcp{citation? or maybe mention Haskell and C programs instead}), so they are often more expressive than other representations like
