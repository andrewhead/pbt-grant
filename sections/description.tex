...
%
By comparison, JUnit version 4 (the most popular version) has 8.4K
stars~\cite{JUnitGithub}---i.e.,
Python's PBT tool has nearly as many stars as the
framework that is widely credited with popularizing unit testing as a technique.

% But the pace of PBT's adoption can be accelerated yet further.

One might wonder if this is ``enough'' success for PBT, but there is actually
significant opportunity for continued growth. The 500K estimate for
Hypothesis users is only 3\% of total Python developers~\cn{}\bcp{I
  actually left that point out for a reason: It undercuts our argument
for potential growth.  After all, Hypothesis already has as many
users as JUnit, which is presumably topped out in the (similarly
sized) Java market.}, and the list of
companies using PBT, while substantial, is very far from whole of the software
industry. And it is not just possible for PBT to grow, it is {\em
  important}.\bcp{Weak transition: These numbers don't imply that PBT
should grow, just that something should.}
...
year. Accelerating the adoption of PBT stands\bcplater{weak} to make a
significant dent
...
something about how PL+HCI is a vibrant, emerging research
area?}\bcp{Mention PLATEAU?}
