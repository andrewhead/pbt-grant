...
infrastructure. \bcp{My math brain finds it hard to imagine exact what
that means.  Would it be accurate to say ``Python's
PBT tool has $\sim$70\% of the stars of Python's entire testing
infrastructure''?  Or can we in some other way simplify it?}
...
two workshop
presentations~\cite{goldstein_problems_2022,shi_towards_2023} and a
paper under submission on the
Jane Street study mentioned above, which inform the
...
themes (Figure~\ref{overview}): \iflater\bcp{Make sure this is an
...
With the growing complexity of software, developers are often faced with code
that they do not fully understand. Modern applications not cut from whole cloth;
they are assembled from complex trees of dependencies that are too deep to hold
in one's head all at once.
And the rise of AI assistants like Copilot and ChatGPT mean that developers are
increasingly faced with code that is easy to obtain but difficult to trust.
\hg{How do we feel about this? Want to say more, or is this enough?}
...
provides a straightforward way to reason about and evaluate these
hard-to-understand programs via their specifications. In PBT
...
The findings of our industry study paint a clear
...
\SUBSECTION{Further User Studies}%
   {sec:survey}{1}{2}{HCI Theory}{PhD 1}{REPL 1}{Head}
\bcp{Or something catchier!  Maybe reintroduce separate section
  headers / management plan items for individual studies.}
%
Preliminary findings from our industry study have already revealed
a number of
...
the things we have learned from Jane Street generalize to other settings.
We will
...
%    Theory}{PhD 1}{REPL 2}{Head}
 ...
 \bcp{Merge what's below with what's above!}
 PBT interaction models...
...
generation vs.{} fully manual generators), and perform a small number of comparative studies to gain
...
Many of the usability improvements suggested by our industry study center
around test input
\gentheme{generation}.
...
Building the idea of free generators, we are exploring a powerful generalization
based on monadic generators called {\em reflective generators}.  We began
thinking about the basic concept during a past NSF project (see
\sectionref{sec:prior}). The first paper on reflective generators won a
...
The overwhelming positive reception of reflective generators has been
encouraging, our first paper only scratched the surface when it comes to
building and evaluating potential use-cases.

The example-based tuning approach we describe in the first reflective generators
paper faithfully follows the tuning procedure used by existing algorithms, which
was important for demonstrating the flexibility of reflective generators, but we
do not claim that that algorithm is the best way to tune a generator.

We think more powerful approaches to example-based tuning are within reach.  The
algorithm ``Inputs from Hell'' algorithm assumes that subsequent choices are
independent, tracking the weights of individual choices independent of the
...
learning model, for example \hg{TODO}. This would make the example-based tuning
...
will state and prove the properties of a generator that imply this property.

\SUBSECTION{Reflective generators for the people}{sec:reflectivefuzzing2}{2}{2}{PBT
  Theory}{PhD 2}{Engineer}{Pierce}

Ultimately, we would like reflective generators to become a
standard representation of generators in real PBT frameworks. In this section we
outline some important steps towards that goal.

Most importantly, it is unclear how reflective generators should be
represented in languages with weaker type systems than the one they currently
rely on in Haskell. In Haskell, the type checker can automatically catch many
common user errors, but this checking relies heavily on type-classes that
quantify over higher-kinded types (e.g., monads). If we hope to implement
reflective generators in frameworks like Hypothesis, in Python, we need to
design a programming discipline that makes the process equally safe and
intuitive. We plan to build on prior work exploring implementing data structures
like monads in imperative
languages~\cite{brachthauser_representing_2021,delaat_pymonad_nodate} to arrive
at a form for reflective generators that is appropriate in languages like
Python, Java, and Rust.

Another barrier to adoption is the extra work involved in turning a standard
generator into a reflective one, since reflective generators are unavoidably a
bit more complex than standard QuickCheck generators. The current design of the
...
specifications.
Our industry study suggested that developers may struggle to write
...
\bcp{To be written...
  \begin{itemize}
  \item Port common / high-leverage scenarios
  \item Expand Ghostwriter with specification inference?
  \end{itemize}
}
...
One surprising finding from our industry study was that
it is quite common for developers to build (or already have) a
{\em model
...
well-documented approach to
  PBT~\cite{hughes_experiences_2016}, but it is not well publicized,
...
effort will be
building and promoting \tyche, an integrated development environment for PBT
...
masters-level course materials for PBT
(\sectionref{sec:1210}).
...
projects. {\em PBT interaction models}~(\sectionref{sec:understanding}): Helping PhD
  1 to build and maintain a minimal PBT framework.
  ...
  languages and PBT ecosystems, with PhD 2.
  ...
  Assisting PhDs
  3 and 4 with building tools for property specification.
%
Most importantly, the research engineer will help to build and maintain
\tyche{}, the IDE for PBT designed by PhD 4.
...
Educational impact has included training graduate and
