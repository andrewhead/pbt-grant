...
\SUBSECTION{Generalizing the Jane Street Findings}{sec:survey}{1}{2}
...
\SUBSECTION{Observing PBT in Practice}{sec:observations}{2}{3}
...
\SUBSECTION{Properties Over Logs}{sec:outpurprop}{1}{2}
...
\SUBSECTION{Models for Modules}{sec:automating}{2}{3}
...
\SUBSECTION{Reflective Fuzzers}{sec:fuzzing}{2}{3}
...
\SUBSECTION{Reflective Shrinkers}{sec:shrinking}{3}{4}
...
\SUBSECTION{Benchmarking}{sec:benchmarking}{1}{3}
...
\amh{Add references to the related work of TraceDiff,
JetBrains object browser interfaces, Telescope, Ply,
Whyline, Gilligan}
...
\SUBSECTION{Evaluating Data Distributions}{sec:evaluating_distributions}{1}{2}
\todo{Add references to related work on debugging,
including FireCrystal, Rehearse, Gamut, Chameleon, Lux,
Burg's Interactive Record/Replay project, Examplore.}
...
\SUBSECTION{Tuning Data Distributions}{sec:tuning}{3}{4}

\amh{Add references to bidirectional editors including
Sketch-n-Sketch, mage, active code completion, livelits}
...
\amh{@HG can we give reviewers a sense of how difficult it
is to tune one's generators using current tools? My sense is
that, for some generators, it can involve a lot of
trial-and-error that is unlikely to pay off. We can more
clearly spell out that the purpose of this project is to
reduce this trial and error by allowing developers to focus
on directly describing the kinds of inputs they want,
rather than tweaking generator parameters and hoping it
leads to the right kinds of input data distributions.}
...
less represented; or (3) manipulating the sections of a pie
chart \amh{Can we downplay pie charts? Pie charts are kind
of looked down upon in the visualization literature, because
they take up a lot of space, while making visual comparisons
and estimations really difficult (people's perceptual
systems are better tuned to estimating and comparing
quantities on a linear scale than a radial one.)} to indicate
...
threshold, \amh{Let's end this paragraph on a more positive
tone. Rather than ``here is an idea, but here is where it
might not work,'' let's phrase as ``a first, generalizable
solution is'' and then begin the next paragraph with ``a yet
powerful solution is\ldots{}''} but a more robust to altering the generator would be better.

\amh{I think it would be more clear to state that the other
approach to changing generator distributions is to change
the generator's code. Then we can state that this is
infeasible to do with conventional generators, but it is
possible for certain classes of changes in reflective
generators.}
...
better represented'' (i.e., by dragging a bar or a pie slice as in interactions
...
obviate the need for expensive \amh{expensive in what sense?
They lead to long-running tests?} filters.
...
\SUBSECTION{Counterexamples as Regression Tests}{sec:counter}{1}{1}
...
\ifdraft
\SUBSECTION{Increasing Assurance with More Tests}{sec:more}{3}{4}
...
\bcp{Add color codes:
  \begin{itemize}
  \item harry: 2.1 and/or 2.2, all of 3, 4.2, 4.3, 5.1
  \end{itemize}
}
