...
\amh{John wants us to note that *the relationships between independently
generated values* may also be important for generating test cases.}
...
\amh{John says that this section may be too vague for non-specialists. He asks,
...
are good uses of this technology.}
...
candidates for sophisticated programmatic manipulation.
...
generators should
produce ``realistic'' inputs \amh{John takes issue with this and thinks that
good inputs are often not realistic. Can we soften this assumption?}; one way to ensure this is to tune the generator so
it produces values that are similar \amh{John takes issue with this, noting that
often you care more about values that can be found quickly than those that are
similar to some other input} to some user-supplied values deemed
...
that Blackwell described.)}\hg{Whatever you think is best}
...
\amh{Consider bringing in ideas from John's prior work on generating
examples:~\cite{ref:gerdes2018understanding}.} \amh{One question that we might
want to better indicate the nuance around is what to do when there is an
arbitrary number of examples---how do we choose which ones to show? John used
heuristics; maybe we can do the same?}
