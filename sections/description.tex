...
The dominant approach to generating inputs and the one we take in this
...
%
And even generators that are guaranteed to produce valid
...
the help of real users, in a style informed the HCI
...
letter of collaboration from Prof.{} Peleg appears in the supplemental
documents.
...
\lstinline|@never@|, and \lstinline|@eventually@| to express
statements about the way the internal state should evolve over
time, but these operators are not available in conventional PBT
frameworks.
...
Our envisioned programmer's assistant, dubbed {\em \GhostChat}, will
wrap existing property generation techniques---including ``classic
GhostWriter''~\cite{ref:ghostwriter} from the Hypothesis project,
...
Under the hood, these suggestions will be generated by a large language model
...
how {\em frequently} each line is executed while running tests, both in aggregate and
