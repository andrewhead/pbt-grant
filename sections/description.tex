...
9~(\participant{9}) said that PBT was particularly easy to apply when one
...
\bcp{Other question we might study is how critical good language
  support (typeclasses, PPXs, etc.) is for PBT...}
...
\SUBSECTION{Properties Over Logs}{sec:outpurprop}{2}{3}{Harry}
...
conventional PBT toolset would fail. This example from P7 is but one of many
situations where important software behavior resides across interacting software
components. \amh{@Harry, fact check this.}
...
that may not be available to the assertion statement.  \amh{@Harry, fact check
this.} \amh{I think I might have better spelled out how this module work, but I
think there is still some unexplained assumptions about what Inner does that
could be helpful to spell out.}
...
calls that can be used to compare module implementations. \amh{This might not
matter, though I am having a hard time understanding this example.}
...
\SUBSECTION{Reflective Generators}{sec:reflective}{1}{1}{Harry}
...
on the needs of the caller.\bcp{Harry: That last sentence is a bit
  mysterious...}
...
constraints, since mutation often produces invalid values.\bcp{Harry:
  Why is this any more the case with PBT than with regular fuzzing?} Reflective
...
following sections. \amh{Maybe we should more explicitly indicate that this
project will provide the enabling technology for more powerful interactive tools
that we will develop later in the proposal.}\bcp{Sure.}
...
inputs.\bcp{Harry: Mysterious.}  For example, we can use them to
...
We mentioned in the Motivation section that developers sometimes think of
shrinking as ``magical,'' but currently the abstraction is too leaky for them to
be comfortable ignoring it.\bcp{??}  The promise of the reflective approach
...
\amh{Our broader impacts section implies that there will be software coming
about from the reflective generators and shrinkers work. The above section
describes only theoretical results. We should briefly describe what kinds of
software (if any) we think we will develop in these
projects.}\bcp{Agreed.  Also, do we need to mention that Hypothesis
shrinking is already kind of magic in this way?}
...
bugs when properties have complex preconditions. \amh{Can we have a more
concrete statement of the goal before talking about prior
work?}\bcp{And is there a citation that justifies that claim?}
...
parser by construction\bcp{Mysterious}; ours will as well, but using a
...
monadic generator, rather than a grammar-based one.\bcp{The reasoning
  here feels a bit tangled.} But the beauty of a monadic
...
\amh{A statement of this level of concreteness needs to appear in the top of
this section.} \amh{Also, what is the software we intend to
develop?}\bcp{Moving this paragraph to the top sounds good (also
...
\noindent{\bf Educational Benefits.}\hg{Feel free to re-format, I just thought
the paragraph*s were taking too much space}
%
