...
no need for it here.}\hg{How do we cite this? HATRA is non-archival}\fi{}
concluded just that. 
  But at Jane Street
...
available or quite easy to imagine.  For example, \participant{9} pointed out
that PBT is particularly easy to apply when one has ``a really good abstraction
with a complicated implementation.''
Participants who were willing to speculate (\participant{3}, \participant{15},
\participant{20}, \participant{22}) guessed that 80--100\% of Jane Street
...
approach education and documentation around PBT may be to focus on
...
Furthermore, more than 75\% of study participants did a
particular kind of PBT, commonly called
\proptheme{Model-Based Testing}. Model-based testing is not so much a scenario
in which to apply PBT as it is a technique for using it, but participants like
\participant{3} thought that better automation and tooling around model-based
testing could make PBT significantly easier to pick up and use.

We discuss plans for work that addresses specification themes in \sectionref{sec:spec}.

Another set of themes concerns the {\bf generation} of
random inputs for property-based testing. Many participants spoke
highly of the \gentheme{Derived Generators} that can be inferred from the type
system (\participant{5} called OCaml's tools for this ``f***ing amazing''\hg{I
really like the impact of this quote, but I'm open to cutting it or finding a
better way to present it (e.g., ``[expletive] amazing'' or I guess just ``amazing'')}
and \participant{30} called them a ``game changer'').
...
failed, participants fell back to standard \gentheme{Bespoke Generators}, which
are far more flexible but proportionally more time consuming to design and work
with. For example, \participant{20} found important bugs with a generator for XML documents,
but they reported spending ``at least a day'' writing it. We are confident
that improving the abstractions available for writing bespoke generators can
greatly improve both the experience of using PBT and its potential for
bug-finding.

If a generated input is deemed to be a {\em counter-example} for a property,
...
We discuss plans for work that addresses generation themes in \sectionref{sec:gen}.

The final category of themes concerns the {\bf validation} of testing
effectiveness. In
...
is true about a close cousin of PBT, fuzz testing. Participants like
\participant{14} and \participant{20}, who used both
PBT and fuzzing tools, ran the fuzzers for far longer
...
strategies like {\em mutation testing}~\cite{papadakis_mutation_2018}; we plan
to explore the user experience around many of these avenues.

We discuss plans for work that addresses validation themes in \sectionref{sec:val}.
...
\hg{What else does this need? Does it work?}
...
\hg{Plan: reverse this section. Start with MBT, then discuss the ML module
system, and then discuss a theory-based approach to make that automatically
testable}
...
expressive and fairly complicated.
...
\label{sec:evaluating_distributions}
