...
We therefore propose a comprehensive, interdisciplinary research program of \emph{usable
...
of the study is that widening the on-ramp to PBT requires pushing developers
toward high-leverage use-cases for PBT. We discuss our plans to teach developers
...
We propose novel approach to testing systems like \lstinline{Inner}, that are
...
programmatic manipulation that enable a host of new approaches to generator
tooling (\sectionref{sec:reflective}). Reflective generators enable new
approaches to fuzzing (\sectionref{sec:fuzzing}) and shrinking
(\sectionref{sec:shrinking}) that reduce developer effort and improve testing
...
%   \caption{Overview of gradually constrained fuzzing.\bcp{Remove figure.}}\label{fig:fuzzing-plan}
% \end{wrapfigure}
...
gain significant testing power. We start with a classic fuzzing setup, attempting
...
\bcp{Compress to one paragraph or so:}
