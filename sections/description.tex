...
motivations that come from GitHub stars :-).}
...
to adopt.\bcp{Do we really want to promise to get into software maintenance?}
...
maintainability in mind\bcp{what does that mean concretely?}, and upon
completion they will be handed off to the
...
\tyche{}, an integrated development environment (IDE) for PBT.\bcp{Is
  it an IDE (all by itself), or is it an {\em extension} or {\em
    plug-in} for users' existing IDE?}

\tyche{} will include all of the functionality designed in \sectionref{sec:val},
including tools that facilitate more intentional PBT
workflows\bcp{what's that?}, help developers tune and
improve generators, and assist in the test-authoring\bcp{sounds like
  unit testing? and we haven't mentioned properties.}
...
\tyche{} into the main Python language extension for VSCode. \bcp{What
would the Python-specific optimizations or features look like?}
...
conferences\bcp{such as...} and
disseminating information via our contacts in industry and the open source
community.

\discussiontarget{Software engineering industry conferences\bcp{e.g.?}}
...
ideological approaches\bcp{??}, making
skill transfer difficult for users\bcp{Do we have any evidence of
  this?}. Thus, we will allocate
engineering time to supporting and improving open-source PBT frameworks.

To address the problem of under-maintained software projects, we will
...
contact with Zac Hatfield-Dodds, the main maintainer of
Hypothesis, and he has a
variety of framework improvements that could benefit from more developer
support; a letter of collaboration from Mr.{} Hatfield-Dodds appears in the
supplemental documents. We expect to reach out to other maintainers
for similar collaborations, looking for high-leverage ways to improve
the state of PBT in other languages.
...
generate documentation that helps developers move from one framework
to another. \bcp{Again, the reader may not be persuaded (I am not,
  yet) that this is a real problem.}
...
high-value PBT frameworks.  \bcp{That seems like a weak claim.}
...
\bcp{This seems much too unfocused---sounds like we are saying }
