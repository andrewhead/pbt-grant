...
(\sectionref{sec:foundation}).
  \item We empower developers to \emph{specify} properties with new approaches 
...
those that represent complex model-based checks (\sectionref{sec:spec}).
  \item We help developers efficiently define and tune test-case 
...
values, test input mutation, and example-based generator tuning, building
  ...
  \item We support developers in rapidly \emph{validating} testing
...
(\sectionref{sec:val}).
  \item We propose an agenda for PBT \emph{education}, grounding this
  agenda in resources for teaching powerful PBT practices in an undergraduate
  computer science curriculum, a tool to help students author their
  first properties, and a comprehensive review of PBT practice
  to help developers learn about the diversity situations to which they should
  put properties to work to validate their software
  (\sectionref{sec:ed}).
\end{itemize}

Each of these research efforts involves a complement of PL and HCI research 
efforts. HCI approaches will be used to concretely define developers' needs, 
...
Here, we describe the essence of the PBT approach, highlighting the component 
tools involved. These tools together form the surface area of technology which 
the research community has leverage in improving.
...
function for a binary search tree (BST):\footnote{The function can be read as follows:%
Assume an arbitrary tree \texttt{t} and an integer%
\texttt{x} to insert into that tree. If the original tree%
is a BST (``\texttt{is\_bst t}''), then it should remain%
a BST after the insertion of \texttt{x} (``\texttt{is\_bst (insert x t)})''.}
\begin{lstlisting}
  prop_insert_correct x t  =  (is_bst t ==> is_bst (insert x t))
\end{lstlisting}
Like most properties, this one is a simple function which takes in generated 
test inputs and evaluates to \lstinline{True} if the test passes, and 
\lstinline{Fales} otherwise.  This function, ``\verb|prop_insert_correct|'', 
checks
that an insert operation on a binary search tree preserves the
binary ordering of the tree.

The value of properties arises in part from their brevity: they often require 
only a few lines to express, yet can validate unlimited
...
In its consideration of generators, this proposal focuses on {\em random} 
generation~\cite{hamlet1994random}. This is in contrast to other
approaches, like enumerative test-case
generation~\cite[etc.]{DBLP:conf/haskell/RuncimanNL08, leancheck} and model
checking. Random generation remains the dominant approach in PBT. Its
effectiveness is often attributed to the
...
a but may be triggered by a sequence of API calls in a particular order, 
regardless of whether they are
interleaved with other API calls. The effect is that testing with large random 
inputs often exposes the same issues as testing with exponentially more
enumeratively generated inputs.

From the perspective of a developer, applying PBT happens entails the following 
