...
\hg{Re-frame as more positive?}
But despite its growing success, little research has been done to understand the
challenges that real-world developers face when actually {\em using} PBT tools.
When have they seen the most benefit from PBT? What slows them down or turns
them away? Could some gain huge benefits from PBT, if only the tools were better
suited to their use cases? If PBT is to continue to grow and find critical bugs
across the software industry, we must take these questions seriously.
