...
{\em foundation} for the latter stages of this project and beyond.  We plan two
...
(\sectionref{sec:survey}), a design probe using a minimal PBT
library (\sectionref{sec:frameworks}), and an observation study to understand
...
users of the Python Hypothesis library; (2) Jane Street, aiming for
a broader set of developers than in the original study; and (3)
...
\hg{Joe says this doesn't aim high enough. He suggests we try to reach X many
users of each of the N biggest PBT libraries---like a lot more than 75
people---and I tend to agree.}\bcp{+1.  X=100?  500?  (Not confident
we can recruit a lot more than 100...)} \amh{Maybe we can settle somewhere in
the middle. I think we want to be a bit conservative, because recruiting
specialized software develoeprs can be rather difficult, and even moreso
when we're recruiting across multiple toolsets (many of which might not
have a way to reach their users).}
...
1}{PhD 4}{Head}
%
\discuss{Cut this task?} \amh{That's fine with me. This can also be easily
(and I think productively) incorporated into PBT interaction models---i.e., our observational studies could
involve comparisons between tools that support PBT with different kinds of affordances.}
%
...
fine-grained understanding of the design trade-offs inherent in PBT libraries
in order to improve those libraries.\discuss{We seem to have standardized
  on ``libraries'' instead of ``tools'' or ``frameworks,'' but are all
PBT tools libraries??  I don't think so!  Or at least not obviously so.}
Of course, a pairwise comparison of {\em all} libraries in {\em all}
languages would be infeasible and
unhelpful; rather, we will systematically review prior art to identify major
dimensions along which PBT libraries differ. For example, some libraries, like
...
preferable? Similarly, some libraries implement properties as arbitrary
...
two ways. First, we will design and implement a minimal, modular PBT library
that can be tuned along the dimensions we want to study.
\iflater\bcp{Can we
give a bit more detail about what this language will be like?  How is it related
to the ``Reference implementation'' idea we talk about elsewhere?}
\hg{We didn't end up talking about a ``reference implementation'' as such. I
imagined we'd build off something like minithesis
\url{https://github.com/DRMacIver/minithesis}, but I didn't want to commit to that.}
\fi
We will
...
\SUBSECTION{PBT interaction models}{sec:observations}{4}{5}{HCI
   Theory}{PhD 1}{}{Head}
%
The studies described above explore large-scale usability challenges, but in order
to build usable tools we also need to study the low-level interactions
between users and their PBT tooling. We will
conduct targeted observational studies to validate our efforts to
design interactive systems (\sectionref{sec:val}).

Understanding user interactions
is a core activity of HCI research and helps to inform robust interface design.
Observation studies can require significant effort to plan and
perform, but they
...
These observations will allow us to evolve singular anecdotes from the Jane Street study
...
threshold'').   \bcp{We say we should do these studies up front, but
  the timeline has them in years 4-5!} \amh{Yes---I think we should be
  conducting these studies throughout, with a spike in the beginning, and a
  trickle throughout the rest of the project term.}
...
% \hg{The last one is useful, but the other two seem too obvious. Can we
% add a bit of color that makes them seem less guessable?}
% \proposecut{Our hope is to achieve simplicity in our theory by invoking a
...
% allow us to refine such ideas further.}\hg{This is where I stopped following.
% The metaphors seem really interesting, but they don't really sit well with my PL
% brain. But I actually think we're good up to that point...}
...
%   success criteria, in fact!)} \amh{Is this better?}

\publicationtarget{Foundations of Software Engineering
(FSE) and Communications of the Association for Computing
Machinery (CACM)}
...
gives confidence that it holds---provided the inputs are ``interesting
enough.''\amh{Elena comment: ``what does that mean?'' (Andrew agrees---can we
factor out ``interesting'' everywhere that appears? I think I used language in
the Interaction section like ``realistic and sufficiently diverse'' that might
fit the bill here.)}
%
However,
...
restrict the set of inputs that are interesting\amh{Elena comment: ``valid?
appropriate?''} for testing. This comes up often
...
discarding invalid inputs\amh{Elena comment: ``valid? approrpiate?''}.
%
...
are easy to apply and can yield good distribution coverage\bcp{What is
``distribution coverage''}, but they are rarely
...
prevents usage errors \amh{Elena comment: ``how?''} (like passing the wrong random seed around). As we will
see, the constrained nature of monadic generators also makes them perfect
candidates for sophisticated programmatic manipulation and interesting\amh{Elena
comment: ``what does interesting mean in this context?''}
generalizations.

To further improve monadic generators, it helps to re-frame generators as {\em
  parsers of randomness}. The usual intuition is that a generator
...
%
This paper introduced {\em free generators}; these generalize the standard
...
generation modulo validity constraints. Free generators look
...
``reflect'' on the choices that they could have made to generate a
given test. The mathematical machinery
...
choices.\bcp{Mysterious.}

Reflective generators have myriad uses. Here are a few...

% \smallskip
{\em Example-Based Tuning.} Earlier we pointed out that good
generators should
...
reflective generators; using it, we can (1) reflect on the choices
that lead to a
...
%
Hypothesis shrinks test inputs by shrinking the randomness\bcp{``shrinking the randomness''?} that produces those
...
randomness requires that the randomness is actually available, which
is not always the case. Reflective generators
can be used to
recover the randomness and thus enable the Hypothesis shrinking algorithm on any
valid input (not just those that we know the randomness for). We can, for
example, automatically shrink inputs provided by the developer or
gleaned from bug reports.

% \smallskip
{\em Other data producers.} Reflective generators can be freely
``reinterpreted'': the same code that specifies random choices can
also be used to {\em enumerate choices} or make {\em guided choices}\amh{Elena
comment: ``Is this the same thing?'' (as ``new choice weights'')}.
...
%
\todo{Poor transition---too self-congratulatory, and unclear about the
  value of what's done and what's not.  Needs to be worded more deftly.}%
Our past work on reflective generators shows promising results, but
their theory is still far from settled.
In particular,
...
implications of. Additionally, reflective generators
...
randomness that makes the
generator make as few choices as possible, and where each choice is
``minimal.''  This is a decent heuristic, but it is not directly
related to the user's understanding of the size of the shrunk
inputs. Ideally, we might want
reflective shrinkers to {\em always} produce inputs that are smaller
by some natural metric on their type, not on the randomness that
produced them; this seems to require a stronger and somewhat more
complex statement about the construction of the generator.bcp{?}
...
bugs when properties have complex preconditions\bcp{Do they work at
  all on components smaller than whole systems??  This is a serious
  gap in my understanding of the related work context...}.
...
inputs. Crowbar does require more user effort\bcp{for what?} than standard fuzzing techniques,
...
under test and exercising other parts of the system. Normally these seeds are easy enough
for the user to write down, since they are simply program inputs, but if
we are instead asking the fuzzer to generate sequences of {\em choices}, then
...
\bcp{The last couple of paragraphs could use a little rewriting---I
  don't think readers will be able to follow.  In particular, the
  switch from fuzzing an input to fuzzing the randomness that led to
  that input goes by too fast, and then the rest is all a muddle.}
\bcp{Also, I am persistently confused by the word ``randomness''---I
  keep wanting to think of it as a single seed such as much be
  provided to a RNG, but it's actually a sequence or tree of actual
  random choices.  I think we need another term.}
...
constraints, since mutation often produces invalid values, but reflective
generators can help with this: The generator can (1) reflect on the
choices that lead to a particular
value, (2) mutate those choices, and (3) re-run the generator (forward) with the new
...
using no bespoke BST-specific code beyond the reflective generator itself.
\begin{figure}[t]
  \centering
...
\proposecut{Between seed extraction and validity-preserving mutation,
  reflective generators have the potential to significantly improve on
  existing tools like Crowbar.}
...
\hg{Joe wants a citation in this section: Andrew? He suggests cognitive
dimensions} \amh{I think we should instead cite~\cite{ref:ko2015practical} which
provides lots of helpful practical guidance on empirically evaluating tools in
lab studies. Though if we want to evolve the design using internal critique,
we could use the cognitive dimensions
framework~\cite{ref:blackwell2003notational}. I would just want to make it clear
that our evaluations would be preliminary with cognitive dimensions (given what
I think might be a shared view that you can only go so far with the dimensions
that Blackwell described.)}
...
\hg{Talk about Rosette}

\discuss{Do we want to talk about LLMs? This will make Hila and co sad, but we
could do it tastefully...} \amh{Elena comment: ``only if you have space; feels
totally unnecessary.'' Andrew agrees.}
...
infrastructure so that users can easily add new things\amh{Elena comment: ``can
you be more specific [than ``things'']?''} along each dimension.
...
specifications.
Our pilot study suggested that developers struggle to write
...
\SUBSECTION{Properties over program tOur user studies were very clearraces}{sec:outpurprop}{2}{3}{PL
...
not be organized in a way that is conducive to PBT. As a unit testing
...
\discuss{Another place to maybe mention LLMs / copilot?} \amh{Elena comment:
``Eh\ldots{} I don't see its relevance''}
...
for PBT in ML-based\amh{Elena comment: ``this came out of the blue?'' [Andrew
comment: Maybe
there is some confusion between ML the language and ML=machine learning?]} languages. It will also provide a basis that others can
...
code~\cite{ref:potter2022contextualized} in prior work. Commonly, PBT libraries
...
\hg{Joe says: Name check expect tests and also Hypothesis examples}
...
for one PBT framework may generalize to others; while not all libraries use
special property syntax (e.g., Hypothesis), all libraries have the potential to
...
\begin{wrapfigure}{r}{0.58\textwidth}
  ...
  would evalate generator source code live (1), showing summary statistics (2),
...
%
\hg{Joe says that the live aspect of the vis plugin needs to come through more clearly}
...
Developers need higher-level ways to describe the kinds of inputs they desire.
...
\publicationtarget{International Conference on Functional Programming (ICFP)}
\amh{Would ICFP be a good target for this? UIST would be a great target too, and
I think would be really excited about this interaction paradigm, so I would
advocate for that perhaps instead.}\bcp{There is no harm listing more
than one target, I think.}
...
the trace is responsible for failure… -- @Andrew what do you think? \amh{I think
this is a great idea. I wouldn't advocate for it for a general-purpose
programming language like Python because slicers are really hard to implement
for those languages. I'm not sure I understand the time-travelling debugger
part, maybe I should take a look at Zac's email.}}
...
the tool will X} \amh{I agree that ``key features'' is clunky phrase that I
wouldn't mind losing. But I think it's really important to help cue when you are
identifying what is novel about a system---otherwise, it's really easy to miss.
So I would find some other way to signal this if we take out ``key features''.}\fi keeping track of the failed input,
...
getting at}\amh{You know how in some test case frameworks there is a diffence
between `.toBe` and `.toBeEqual` relating to the strictness of comparison?
That's what I was referring to. Does this have an analog in languages like
Haskell and, if so, is it convincing at all?}\fi. We also hope the results of this work
can inform the
...
to supporting open-source libraries for PBT (\sectionref{sec:nurturing}).  Our
education-focused activities will include a catalog of high-leverage PBT
use-cases (\sectionref{sec:whento}) and an undergraduate\bcp{and masters?!} curriculum
(\sectionref{sec:1210}).

\discuss{Do we need to make a section for CACM?}
...
\begin{itemize}[noitemsep]
  \item \sectionref{sec:frameworks}: Helping PhD 1 to build and maintain a minimal PBT
    framework for use in comparison studies. \amh{Elena comment: ``after
    reminding myself of 3.2, I didn't realize this would be
    necessary.}\bcp{This section is on the chopping clock anyway?}
...
ideological approaches (we described a few examples
in~\sectionref{sec:frameworks}), making
...
Haskell, and the PBT libraries in Rust seem like high-value libraries worth
...
between PBT libraries. Where possible, we hope that our comparison of PBT
tools (\sectionref{sec:frameworks}) provides us with enough evidence to convince some
...
\todo{What about masters students???  Arguably even better opportunity
  for impact!  (I changed the intro to promise both ugrad and masters here!)}
 \amh{I think that's a good idea +1}
\todo{Transition}
%
...
\publicationtarget{Koli Calling}
\bcp{Will everyone know what that is?}
...
open source license\amh{Elena comment: ``how is this different than
``open-source distribution'' mentioned at the top of the paragraph?} to serve as models for similar tools for other programming languages and environments.
