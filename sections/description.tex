...
\amh{Simply replacing the phrase ``Jane Street study'' with ``our
preliminary study at Jane Street'' will probably go a long way to
making us look less like we are overfitting to that study.}
...
infrastructure \amh{instead of ``entire,'' maybe ``de facto''? ``Entire''
suggests to me a sum over all Python testing frameworks}!
...
\amh{Another chance to avoid the perception of overfitting to Jane
Street---can we phrase this in a way that suggests this is a first study?}
In a recent need-finding study with PBT users at Jane Street
...
\amh{I think we need \emph{some} reference to the figure. I am not
sure it will be clear from context alone that the purpose of this figure is to
summarize the main research threads and how they relate to each other.}
...
% \begin{enumerate*}
  \item We will establish a firm \amh{Maybe ``firm'' is too strong a word given
    that we only have 1.5 years of very targeted studies we will be running} \thread{foundation} for HCI-informed research on PBT,
    building on our past need-finding study and to confirm \amh{Can we ditch
    ``confirm,'' as another way to avoid suggesting overreliance on Jane Street
    study?} and generalize
...
languages for expressing properties, tools that simplify authoring
    ...
    \amh{Update (to whatever wording we choose for the project summary)}
(\sectionref{sec:spec}).
...
enable new ways of visualizing and understanding random distributions
    over test inputs and pinpointing the cause of test failures \amh{cut,
    replace with ``tuning those distributions''}.
(\sectionref{sec:val}).
%
\item We will support the \thread{diffusion} of PBT tools and
methodologies from academia into industry by\iflater\bcp{This used to
  also say ``helping
improve open-source PBT frameworks'' but I think that's too ambitious}\fi{}
leading the construction of a
    comprehensive \amh{We should cut ``comprehensive''} open-source IDE for PBT,
    dubbed \tyche. \amh{We should mention efforts to promote adoption via
...
\publicationtarget{See later sections. Findings from these studies will serve as
``formative study'' sections of respective technical publications that they inform}
...
{\em fuzzers}~\cn \hg{@Harry: See Shriram's comments on the ICSE paper for stuff on this
...
\bcp{publication targets also missing here}
...
%
\amh{Update this section: remove publication target; this work
now has two purposes (1) we will make the benchmark available to the
community, further developing it so it can be used to evaluate
existing PBT tools and (2) we will use the tool ourselves to
evaluate the tools we develop elsewhere in this proposal---specifically
for Sections 4.1 and 4.2.}

\todo{There may be additional
    interesting things to do in the benchmarking space  [@Benjamin]}%
...
\bcplater{This subsection is OK, but it could be stronger on details
  like what LTLTL actually looks like.}\hg{Ideally it would look like exactly
  the kinds of properties above. Or were you looking for a formal
  definition?}\bcp{No, not formal.  Just a couple sentences with a bit
more detail.}
...
% \bcp{@Harry and @Andrew: I really like the idea of doing something
%   with LLMs, but I don't think the vision in this section nails it, in
%   part because it isn't clear about where the focus is.  The property
%   generation techniques listed are {\em very} different---I don't see
%   much reason to believe that interacting with them through a common
%   generic UI is going to work well.  Furthermore, the ``use LMMs to
%   generate specs'' idea actually feels like the main proposal here,
%   but there's hardly any detail about how that might work, what the
%   challenges would be, etc.}
...
\amh{We aim to have a significant user community and significant
copntributions to \tyche{} outside of Penn. Our goal is to
transition \tyche{} into shared ownership open source project
with significant contributions coming from outside Penn by the
end of the 3 years.}
