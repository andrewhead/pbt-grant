...
but it is not enough: new ideas are needed when the target of simplification
is code with recursion and/or manipulating complex data. We propose a semi-automatic
...
simplifications in the background. Key mechanisms
would include (1) displaying
the valid ways an input can be pruned,
using reflective generators to understand which parts of the
structure can be changed while maintaining preconditions\bcp{are preconditions
  really about ``parts'' of the structure?}, and (2) automated
shrinking after each action to suggest a small number of divergent shrinking
decisions a developer may wish to try next.\bcp{A PBT expert may
...
found this technique does not actually identify the fault discussed in
Hypothesis' Quick Start
documentation!\bcp{Not sure I get the point of referring to H's QS
  docs...?  I tried a bit of rewriting, but.} Could a tool highlight divergences in other more useful ways? PI
Head was part of a team that designed views of trace divergences for student
...
that supports developers in creating unit tests from
counterexamples. In this flow, a developer's
editor creates a provisional unit test. The developer is
guided through a sequence of decisions centered around inputs,
operators\bcp{what operators?}, and
...
we will prioritize decisions that are time-consuming and that vary by
...
community.\bcp{And uptake in the community?}
