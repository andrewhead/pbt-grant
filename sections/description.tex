...
\amh{What follows is a first attempt to try to do a harder sell of the
transformational nature of the project:} To make PBT not just useful by
...
The conceptual advances needed to improve the usability of PBT
...
work\ldots{}} This project puts forward a comprehensive program of innovative
...
statistics. Current PBT practice is severely limited in the kinds of
...
refereed conference demo~\cite{ref:goldstein2023tyche}, and a
paper under submission on the
Jane Street study mentioned above; all of these inform the
...
\end{wrapfigure}
%
We propose an interdisciplinary program of research in
...
improving open-source PBT frameworks and building a
...
hard-to-understand programs by validating them, semi-automatically,
...
the \lstinline{insert} function on binary search trees:
\begin{lstlisting}
...
The research agenda we propose focuses  on {\em random}
...
\bcp{OK, I crunched this section down to a page.  I don't want to go
  too much shorter because it is doing a lot of work introducing the
  themes that we return to later...}
%
Our research agenda is informed by the findings from
a need-finding study that we carried out last year at Jane Street Capital.
Our purpose in this
study was to understand the usability challenges
that must be addressed
to boost adoption of PBT in
industry. The study data consisted of thirty semi-structured interviews
with (1) developers who use PBT and (2) maintainers of PBT tools, and
a paper describing our findings is under submission to ICSE.
...
Several sets of themes emerged from this study.  One concerned the
\thread{generation} of random inputs for PBT. Developers spoke highly
of the \gentheme{Derived Generators} that can be automatically
inferred from the OCaml type system but identified deficiencies both
...
%
When derived generators failed, participants fell back to
...
%
Furthermore, when a generated input turns out to be a counterexample
that triggers a property violation, the developer will need to inspect
that counterexample to find and fix the root problem. Developers often
implemented code for \gentheme{Shrinking} counterexamples to discover
...
A different set of themes themes revolved around properties
themselves, i.e., around \thread{specifications}.  PBT is often described as
...
A further set of themes concerned the \thread{interaction} between
developers and their testing environments---especially their
strategies for \evaltheme{Evaluating the Effectiveness} of their
...
distribution of randomly generated inputs.

Significant engineering and pedagogical effort will be needed to
amplify PBT's \thread{diffusion} into the broader community. PBT is
...
\bcp{I tried to make the opening of this section give a better sense
...
much from the top of section 2...?}

The findings of the industry study described above paint a clear
...
characteristics---i.e., teams consisting mostly
...
usable just for them would already be a significant step in the
...
In this section, we describe four\bcp{?} planned studies that aim
...
\SUBSECTION{Broader User Studies}%
   {sec:survey}{1}{3}{HCI Theory}{PhD 1}{REPL 1}{Head}
%
...
\SUBSECTION{Observational studies}{sec:observations}{4}{5}{HCI
    Theory}{PhD 1}{REPL 2}{Head}
\bcp{Wonder whether there is still room for cutting / compressing
...
\sectionref{sec:reflectiveusable}\bcp{broken link---should it point
  elsewhere, or should we consider cutting more here?}) and
...
\bcp{Fix this paragraph.}
We propose {\em reflective
...
tooling (\sectionref{sec:reflective}\bcp{broken}). We explore how reflective generators can
...
preconditions\bcp{because they don't generate enough valid
  inputs?}. Slightly more
...
satisfy. \bcp{what are they good at and what are they bad at?}
...
gleaned from bug reports.  \bcp{Wonder if that is getting too much
  into the weeds...}}
%
...
As new strategies for guided random generation and
enumeration become available, they can be used to improve reflective
generators.

\SUBSECTION{Algorithmic advances for reflective generators}{sec:reflectivefuzzing}{2}{2}{PBT
...
distribution of values, we should likely take that contextual information into
account. We propose modelling choice weights with a more sophisticated machine
learning model, for example \hg{TODO}. This should make the example-based tuning
procedure far more accurate, allowing it to much more accurately replicate
important features of complex input sets.

\hg{These three paragraphs aren't perfect yet. In particular, I want to talk
about interpretability of reflectives playing a role here.}
...
property. \bcp{That ends a bit abruptly...}

\SUBSECTION{Reflective generators for the people}{sec:reflectivefuzzing2}{2}{2}{PBT
  Theory}{PhD 2}{Engineer}{Pierce}
%
...
We have built the first version of a robust empirical evaluation
framework for
...
effectiveness.  By ``easily,'' we mean that we take on the burden of
collecting data and analyzing the results.  The tool
can help users and developers evaluate a given tool based on (1) the
...
Some initial comparisons using this framework appeared as an experience
report to ICFP 2023, but many crucial features (e.g., coverage
measurement) have not yet been implemented.
...
things: writing generators (as discussed above) and envisioning and writing
specifications, which we address next.

Developers struggle to write
properties for their programs, sometimes due to poorly abstracted code and
...
property language that operates over program traces (\sectionref{sec:outpurprop}\bcp{broken/gone}) and
the second with mixed-initiative interactions in which the user and the computer
...
{\color{gray}
\bcp{Change this to just a footnote in the Ghostwriter section.} Properties over program traces...
...
event logs. Consider
the case of P7, who was testing
...
we hope to choose a simplified set of temporal connectives that are
intuitive for users.
%
...
\SUBSECTION{Ghostwriter++}{sec:interactive}{4}{4}{HCI Practice}{PhD 3}{PhD 4}{Pierce}

\bcp{ (name?)}
\hg{
  Here's the outline I had in mind:
  \begin{itemize}
    \item The JS study highlighted high-leverage scenarios
    \begin{itemize}
      \item Invariants for modules
      \item Model-based / differential testing (generalization of invariants)
      \item Round trips
      \item Classical math properties
    \end{itemize}
    \item Some of these scenarios are handled by ghostwriter, we'll add support
    for more
    \begin{itemize}
      \item Invariants / model-based --- incorporate ideas and techniques from Mica
      \item Others ???
    \end{itemize}
    \item What about bringing more powerful tools to bear?
    \begin{itemize}
      \item TOGA demonstrated that language models can be used to generate
      correctness assertions---could we do this with properties?
      \item We may also be able to do generators (HG: I don't have a strong
      story here without bringing in the whole generator synthesis stuff, which
      I don't want to do. But we can mention it)
    \end{itemize}
  \end{itemize}
}
...
% in situations that actually come up in their work. We will explore how
% programmers can engage in an iterative loop to
% compose properties in collaboration with their tools, focusing on
% first encounters with PBT in an educational setting.

% We will design a novel mixed-initiative~\cite{ref:allen1999mixed} workflow where
...
% smith_discovering_2017}. Our aim is to explore how student and tool
% can collaborate to arrive at properties that
% are not just {\em accurate} descriptions of correct system behavior, but
% also capture {\em important} aspects of behavior. To achieve this aim,
% it will require
% designing interactions for developers to specify desired properties
% directly, plus extensions to property extractors that incorporate this
% information. We envisage
% interactions that help students steer a property extractor by (1) selecting
% specific functions or procedures that they want to check,
% and then indicating the kinds of properties they wish to test by (2)
% writing unit test cases or REPL assertions
% that represent special cases of more general
% properties. This interaction would be
% mixed-initiative, permitting a user to refine generated properties by providing
% additional test cases that should be satisfied or rejected. Such interactions
% would be prototyped on top of an open source property generator tool like
% QuickSpec~\cite{ref:claessen2010quickspec}, using user inputs to steer
% generation, and evaluated in Penn's upper division Haskell
% course (CIS 5520).
...
%
\bcp{Cite Ernest's SRC poster.  Compress if needed.}
%
One surprising finding from our industry study was that, at least
in this environment, it is quite common for developers to build (or
already have) a
{\em model
...
well-documented approach to
  PBT~\cite{hughes_experiences_2016}, but it is less well publicized,
  nor is it supported as well as it could
be by existing tooling.
%
With this in mind, we plan to build a tool to automate
model-based testing for systems written in languages with strong
{module systems}, like OCaml, Standard ML, and
Scala~\cite{macqueen_modules_1984}.\bcp{Why restrict ourselves to
  these?  Java, Rust, Haskell, etc. should also have enough features...}
We will start in OCaml because our existing relationship with the OCaml
community makes it easy to rapidly get feedback on findings and tools.

Model-based testing is easiest in the case where
the component under test and the model are both pure functions.
When we are  testing a {\em
...
Figure~\ref{fig:sigs} requires wiring together calls to its functions
in a well typed way---e.g., testing \lstinline{drop_n}
requires an integer and a string, each of which must be either
...
Things get even more interesting when modules contain {\em abstract
  types}, as is
the case with \lstinline{Set} in Figure~\ref{fig:sigs}. Now some of
the types mentioned in the signature cannot be
generated on their own; their values must rather be obtained by
...
testing.  There is one useful piece of prior work on testing modules with abstract
types\iflater\bcp{describe the results and why they are not the whole
story...}\fi~\cite{hou_favonia_logarithm_2022}, but the problem has
not been thoroughly explored.

To address these challenges, we will take a foundational
approach, breaking down
...
with human users. Success will be measure by the interactions' ability to
reduce
...
\SUBSECTION{Tyche: Exploring Data Distributions}{sec:evaluating_distributions}{2}{3}{HCI
  Practice}{PhD 4}{REPL 8}{Head}
%
\todo{Cite the UIST demo -- proof of concept for feedback.  We will do
more!}
Unlike conventional unit testing, where developers
...
Our research efforts will focus on contributing the interaction primitives and
 unifying environments necessary to answer questions like these. We
...
\bcp{Merge the following with the above}
Tuning data distributions
...
failed tests, but never by successful tests. But we
found this technique to be inconsistent, even on examples provided in the
Hypothesis documentation.
Could a tool highlight divergences in other more useful ways? PI
...
% One pain
% point reported
% by informants in our Jane Street interviews is that it requires
% considerable work to
% transform a (repaired) counterexample detected by their PBT tools into a
% regression test, even though much of this work
% feels mechanical.

% Much of this work could be automated away. After all, a repaired
% counterexample
% is just an input to some component plus (as specified in the property)
% a way of checking that the produced output matches expectations.
% In theory, this should easily be transformable into a unit test.
% In practice, there are numerous issues to deal with.
% Consider: (1) an input may be so large it belongs in an external
...
% that supports creating unit tests from
% counterexamples. In this flow, a developer's
% editor creates a provisional unit test. The developer is
% guided through a sequence of decisions corresponding to different ways
% they may wish to ``write out'' the test
% case.  Each decision results in a re-rendered test (e.g., where inputs are
...
% we will prioritize  hard-to-automate decisions---ones that are time-consuming
% or vary by context.

\publicationtarget{Foundations of Software Engineering (FSE)}

\SECTION{Diffusion}{Bringing Innovations to PBT Practice}{sec:diffusion}
...
educational efforts and the development of tools
...
(\sectionref{sec:ide}).  Then, we will conduct activities to educate
programmers in effective application of PBT by developing a catalog of
high-leverage PBT use cases (\sectionref{sec:whento}) and undergraduate- and
...
\SUBSECTION{An IDE for PBT}{sec:ide}{3}{5}{Tech
Transfer}{???}{}{Pierce}
The projects in \sectionref{sec:val} will explore a wealth of new modes of interaction
for PBT. Their potential impact will be magnified by
...
of VSCode's status as the most widely used editor according to a 2022
survey~\cite{noauthor_stack_nodate} and its support for every language
with mature PBT tooling. The IDE will be developed in consultation with
our consultants on the Hypothesis team (see our collaboration plan) to
...
project, including co-supervising undergraduate and masters
researchers.
...
of PBT.  Our preliminary industry study has been carried out with the
