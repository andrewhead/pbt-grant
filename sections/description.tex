...
\begin{itemize}[noitemsep]
  ...
  industry by conducting additional formative user
  studies (\sectionref{sec:foundation}).
  ...
  of use cases with the ability to write properties over logged output (\sectionref{sec:spec}).
...
Before discussing our motivating study, we should briefly clarify what PBT is
...
\begin{enumerate}[noitemsep]
  \item Define one or more properties that should be true of the program under
  test.
  \item Design (or otherwise obtain) {\em random input generators} for the
  values that the properties take as input.
  \item Check the properties with generated inputs, using infrastructure
  provided by the PBT framework.
  \item If counter-examples are found, make sense of them and determine the
  source of the bug.
\end{enumerate}
Each of these steps presents opportunities to improve the user experience of
PBT; when applicable, we will make it clear which of our projects targets which
...
\subsectionstar{Preliminary Themes} The main theoretical output from
...
Help me figure out what to prune!}

Our preliminary study at Jane Street paints a clear picture of the
benefits and challenges of PBT in one specific context.  But to fully
understand its potential impact across the software industry and the
factors that may limit its adoption, we need to cast a wider net.
%
In this section, we describe two planned studies, drawing on mixed
methods in the service of producing a comprehensive and actionable
agenda for future research, in this research project and beyond.
Concrete tasks include a written survey to assess the generality of
these needs and obstacles and to identify potential for adoption of
PBT tools (\sectionref{sec:survey}), and an observation study to
understand particular tasks involved in PBT to guide the design of new
algorithms and interactive tools (\sectionref{sec:observations}).
...
Our Jane Street study has revealed a number of opportunities to
improve tools for property-based testing. To better understand which
of these opportunities are most important for the research community
to explore, we will conduct two surveys, each with a broad sample of
developers. These surveys aim to (1) determine which obstacles from
the interview study are most representative of widely-experienced pain
points with PBT tools and (2) characterize the potential benefits of
better PBT tools to the software industry as a whole.
...
me.}} (see the attached letter of support). Recruiting from this
population allows us to overcome
...
\bcp{This section needs to be clearer about expected outcomes /
  contributions of these studies.} \amh{Are we any closer to hitting the mark on
clarity of expected outcomes / contributions with this expanded text?}
...
\bcp{This section should close with some kind of transition that
  launches us toward the next section.} \amh{How about the below?}
...
\SUBSECTION{Proposed Work: Properties Over Logged Output}
...
Property-based testing poses kinds of challenges in evaluating test results that
are unique within software testing and, in our opinion, currently without
solution. These problems arise from the multiplicity of tests\ldots{}
\amh{Continue from here. The challenges that arise from working with generators
are that: (1) a developer has not manually written the inputs, so they do not
actually have an understanding of those inputs as a natural byproduct of writing
the test code; (2) the generated inputs can be complex; (3) a developer does not
care just about singular inputs, but rather the distribution of generated
inputs, that it covers a sufficiently interesting and representative space of
inputs; (4) as with conventional debugging, it can be difficult to understand
the cause of software failures---though here PBT provides a unique opportunity,
because failed inputs might be best understood in terms of deviation from
behavior for nearby successful inputs; (5) developers needs to make decisions
about how much computer ``budget'' to allocate to generating and testing inputs.
Our goal in the upcoming work is to develop a vision of what great testing tools
look like for a multiplicity of generated inputs.  This is an area that has not
yet received attention in the area of HCI for programming tools, and one that is
fruitful with exciting areas to explore and with high potential for impact.}
...
\SUBSECTION{Proposed Work: Visualizing and Tuning Generator Distributions}
...
\SUBSECTION{Proposed Work: Debugging Support for Understanding Counterexamples}
