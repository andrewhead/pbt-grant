...
to assemble these
techniques and others together into a unified tool, {\em \GhostChat}\hg{FWIW I'm
not the biggest fan of this name... If it's not a chat interface, I propose we
go back to Ghostwriter++.  But either way we should mention a name down below
too}, for
...
What do we know about prior work fitting this description?}\hg{Certainly
Hypothesis---it doesn't automate anything, but it does make writing stateful
tests like this pretty easy. I'll ask Ernest}, (2) to
...
\bcplater{This subsection is OK, but it could be stronger on details
  ...
  the kinds of properties above. Or were you looking for a formal definition?}
...
One might be tempted use these tools to produce properties directly, but that
would be quite risky. Test suites are part of an application's {\em trusted
computing base}~\cn, and, as such, should be carefully considered and obviously
bug-free. Large language models are known to hallucinate~\cn, so we would not
recommend giving them direct access to an application's test infrastructure.
...
However, we do think LLMs have a role to play!
We propose AI-powered tools that help developers with property ideation
while still ultimately using trusted
property generation patterns.
The envisioned programming assistant would wrap existing property generation
techniques---including \Mica{} and LTLTL (proposed in the prior two sections),
QuickSpec~\cite{ref:claessen2010quickspec}, and
GhostWriter~\cite{ref:ghostwriter}---under APIs that allow them to be invoked by
an LLM programming assistant AI.
...
exists, allow the programmer how to run the appropriate tool automatically.
...
documentation.\hg{What was that thing Andrew Zhu is working on? We can cite that
here}
...
provide visibility and control (i.e., the ability to change or halt\bcp{really?})
...
aggregators like \lstinline{max_by}\bcp{what would this do to a
  log??}. Such features can be
