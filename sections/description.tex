...
2023, but there is still more to do. First, we will extend Etna with
...
logics such as Linear Temporal Logic (LTL).  The first step
is logging specific values of interest using lightweight annotations
within modules like \lstinline{Inner}. The next is a language for
...
% }
%
High-leverage PBT scenarios provide clear recipes that developers
can use to decide when and
how to apply PBT.  But these recipies currently require some work for
developers to understand and internalize.  Ideally, a naive developer with code that is
easily ``PBT-able'' could jump into the process with automated help.
We will experiment with providing such help through a combination of automated tools and workflows
in the IDE.

The Hypothesis PBT framework in Python currently offers {\em
  Ghostwriter}, a limited set of tools for automating the process of
writing properties. It implements automatic property generation for
round trip, equivalence, and other simple mathematical properties.  We
would like to expand the capabilities of such tools to automatically
infer properties in a much wider set of scenarios, via rich
interactions with the user.

\bcp{Needs rewriting from here...}
