...
values, test input mutation, and example-based generator tuning, building
  on a ``reflective'' approach to generation (\sectionref{sec:gen}).
...
%
\hg{Took a stab}
Note that early designs and prototypes for this work have already been funded by
\hg{\dots}---so far, the theory is sound. We have built an early demo
implementation and written a draft paper that both show promise. Still, many
details remain to be ironed out, and, while exploratory empirical evaluations
look promising, more thorough experimentation will be required.
\bcp{Talk about what's done and what's to
  be done.}
...
projects will be pursued using HCI methodology,
...
\begin{wrapfigure}{r}{0.6\textwidth}
  ...
  \caption{An envisioned tool for evaluating input data distributions.}\label{fig:gen-vis}
\end{wrapfigure}
...
%
Education can be a force-multiplier that helps make technologies more usable.
When tools or techniques become standard in undergraduate curricula, the
students can go on to help bootstrap adoption of those ideas in industry.  PBT
would be at home in a number of standard computer science courses--- compilers,
functional programming, and even early introduction courses---but we believe the
most promising place to start will be a data structures course.

Teaching PBT alongside data structures has
precedent~\cite{wrenn2021using,nelson2021automated}, and is natural for a number
of reasons.  Data structures is often taken early in the curriculum, so habits
can carry over to other courses even where PBT is not explicitly used. In
addition, data structures often have ``obvious'' properties that can be a great
place to start with PBT (e.g., algebraic specifications, inefficient reference
implementations, see \sectionref{sec:whento}).

Concretely, we plan to design a model for adding PBT to data structures courses
and implement this model in the University of Pennsylvania's CIS 1210. We will
add a lecture early in the course on PBT and then make PBT a requirement on
future homework assignments.  Materials we produce will be made public so other
universities can copy and adapt our model to their own courses. We will also
publish a paper in a computer science education conference that advertises and
explains our methods.
...
\end{enumerate*}
These properties cover a range of
...
This was a strong signal in the pilot study\bcp{I have a persistent
...
us blur that distinction?} and a great opportunity for new
