...
no need for it here.}\hg{How do we cite this? HATRA is
non-archival}\bcp{Cite the arxiv version and say it was presented at
the workshop.}\fi{}
concluded just that.
...
We discuss plans for work that addresses \evaltheme{validation themes} in \sectionref{sec:val}.
...
\SUBSECTION{Surveys to Generalize Findings}{sec:survey}{1}{1}
...
\SUBSECTION{Observations of PBT in Practice}{sec:observations}{2}{2}
...
\SUBSECTION{Writing Properties Over Output Logs}{sec:outpurprop}{2}{3}
...
One Jane Street developer (\participant{??}) described their experience testing
a system (we'll call it System A) with messy abstraction boundaries. In this
case, System A was hard to test because it was primarily used as a sub-component
of a larger System B: System B would take in simple data, and then at some point
pass a large, complex data structure to System A, which would do some work and
hand results back to System B. PBT of System A was therefore hard, because
generating valid examples of the large data structure that System A takes as
input required deep knowledge of the internals and types of System B that the
developers of System A did not have access to, and because the interface was too
complex to write straightforward properties about.

Here is a straightforward approach that might help in this scenario: add some
logging calls to System A to record when interesting events occur, then write
properties over the logs that are produced when it runs as part of System B.
For example, the developer might ensure properties like
\begin{lstlisting}
  msg.length < 100 ==> never (overflow = true)
  eventually (processed = msg.length)
  processed <= next (processed)
\end{lstlisting}
that demonstrate that internal variables like \lstinline{msg},
\lstinline{overflow}, and \lstinline{processed} evolve correctly over time.
These properties can be tested without ever running System A on its own: we can
generate inputs to System B, have System B call into System A (which logs out
variable values), and simply monitor the logged values to check the desired
properties.\hg{I think this is an improvement, we can discuss}

% We can use meta-programming to automate this
% process, so a user need only mark the variable at its declaration site
% to log all updates to it.\bcp{Can OCaml actually do that??}\hg{I don't see why
% not... ppxs are arbitrary code transformations}\bcp{How would it deal
% with, for example, aliasing (i.e., multiple pointers to some
% ``watched'' location, with different names)?}
...
``mere matter
of engineering,''\bcp{how hard is it?}\hg{Not sure, but I doubt it's a
theoretical lift...} but the language for properties
...
broader set of programs.\bcp{Weak ending, and this whole section is a
bit vague.}\hg{Would love some input. It's vague because I really haven't had
time to flesh out this idea}
...
was specific to the problem at hand, so the hard work of setting up these tests
has no hope of being reused from one project to another, but the logic behind
the harnesses is ripe for automation.
\bcp{But now that they're done, why is more research needed?}\hg{Is it
not compelling that developers continue to waste time on the same automatable
task? I thought that was a slam dunk...}\bcp{We're not being explicit
about exactly what's missing that makes people have to reimplement
things all the time---if what was missing was just the things, then
once they'd been implemented we'd be done.}
\hg{Better? I think it wasn't clear that ``harness'' doesn't mean
``generalizable library'' it means ``domain specific thing that won't help me
next time'}
...
solve, and then make it clear when we've solved the problems.}
...
and generator power.\bcp{Better!  We can make it better yet by being
  explicit about where these tradeoffs happen in the discussion that
  follows.} The existing solutions 
...
\bcp{Why are developers not happy with
  these approaches / tools?}
...
\bcp{If we're going to include the benchmarking project at all (and I
  ...
  suite that we and others might build later.  (We can and should
  mention that we've been discussing potential designs with UMD.)}
...
\hg{Hila had a similar idea that she wanted one of her
undergrads to work on, we should make sure we don't step on
toes}\bcp{IMO the danger of stepping on toes at this point
is minimal---the time to worry about (and check) whose toes
are out there is when we actually start doing the
work.}\amh{@BCP, should we be concerned about Harry's
concerns if Hila is a prospective reviewer of this proposal?}

\SUBSECTION{Evaluating Input Distributions}{sec:evaluating_distributions}{3}{4}
\newcommand{\genvis}{GenVis}

\amh{Andrew's planning notes for this section: What takes
this project beyond a typical visualization to a new kind of
power tool for conducting multi-input tests is: (1) direct
hooks into functions defined in the local scope which can be
used as directives for visualization; (2) live feedback as
the generators change; (3) support for common programming
structures (i.e., trees, graphs); (4) the ability to look up
individual examples; (5) the ability to ``hop'' between
similar examples that succeeded or failed. I think that the
combination of 1, 2, and 4 together might be enough to
constitute a ``novel approach'' to interactive debugging.}
...
(think lists, trees, and other algebraic data types). As a concrete example,
...
visually may be hard if the values are large.  Furthermore, writing generators
is already considered by developers to be a burden, so any tool in this space
needs to be incredibly automatic and easy to use.
...
validated by the user. In the case of the \lstinline{log} type above, \genvis{}
would consider simple functions like \lstinline{length},
destructors like \lstinline{head}, \lstinline{id}, and \lstinline{payload}, and
even aggregators like \lstinline{max} and \lstinline{avg}. Then
\genvis{} will then use lightweight program synthesis to compose and
combine these functions to get features. It may choose to show the
\lstinline{length} of a log, but also the \lstinline{max (length . payload)} and
even \lstinline{(length, max (length . payload))} (plotting length versus
the maximum payload length, in, e.g., a scatter chart).
If there are features that the user notices should be extracted, but that the
system cannot come up with itself (e.g., \lstinline{ids_unique}) the user can
write it themselves and add it to the \genvis{} interface.
All of the features, base and derived, will be made available to users
in a variety of charts and graphs.\todo{I don't think this is at all clear, but
I wanted to get something down on paper to talk about}
...
some generated values that can be used to help the developer spot-check the
generated data.  The specifics this are not yet certain, since of visualizations
used for individual values may need to depend on the domain, but we have some
ideas. For some kinds of structures, it may be useful to present the user with
DOT graphs~\cite{ellson_graphviz_2002} that represent data types; such graphs
could be generated via meta-programming and rendered for the user. Another
option is to simply present the user with a JSON representation of
the data that they can explore via mouse interactions (e.g., clicking to expand
or collapse sections) or even via tools like \lstinline{jq}~\cn{}.
...
features. We will evaluate \genvis{} in a small user study (around 15 participants),
...
\SUBSECTION{Tuning Input Data Distributions}{sec:tuning}{1}{3}
...
\SUBSECTION{Transforming Counterexamples into Regression Tests}{sec:counter}{3}{4}
...
\SUBSECTION{Running More Tests with Less Work}{sec:more}{3}{4}
% \SUBSECTION{Integration with Continuous Integration Workflows}
...
% \gantttitle{Ongoing}{1}
  ...
  \input{main.workplan}
\end{ganttchart}
  \caption{Plan of work.  \bcp{It's too big probably.  :-(  We can
