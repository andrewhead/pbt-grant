...
The study consists of semi-structured interviews, exploring
...
of analysis have been completed, but full-scale analysis has not yet
begun. For more details on the study parameters and the current state
of the project, see \sectionref{sec:foundation} below.
...
available or quite easy to imagine. This suggests a more effective way
to approach education and documentation around PBT, focusing on
...
where it could be with a little more research effort.\bcp{Quick
...
property, highlighting a bug in the code, it should needs to be easy to use that
...
effectiveness. In
general, it can be difficult to know when ``enough is enough'' when it
comes to any sort of
...
\SUBSECTION{Ongoing Work: Need-Finding Interviews with Jane Street Developers}
\label{sec:interviews}

\amh{I attempted to fix conflicting verb tenses to better reflect what
we now have done (i.e., we have finished conducting the interviews). This still
reads a bit clunky, and the text would probably benefit from some smoothing.}
...
\textbf{Method}. We conducted semistructured interviews with
approximately 30 developers. Questions focused on tasks these developers wish to
...
\textbf{Population}. Ideally, these interviews would have included both current
users
...
attached letter of support), with whom we have begun the expanded interview
study. Interviews have been conducted with 30 stakeholders, including both
...
% \SUBSECTION{Proposed Work: Characterizing the Potential for Impact in Improved PBT Tools}
