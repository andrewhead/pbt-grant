...
\hg{Got rid of personnel and put this here. Thoughts?}\bcp{I like it
  here.  Needs a bit better transition to what comes before and after.}
PI Head
...
Here is a straightforward approach that might enable much better testing
of poorly abstracted code without giving
up expressiveness: add some logging calls to the
...
values satisfy the invariant.\bcp{Not very convinced by this
  example---it's way too abstract.  What real examples do we actually have in
mind?}  We can use meta-programming to automate this
...
never decrease).
This would be difficult to do with debug assertions\bcp{who said
  anything about debug assetions??}, since state would need to
...
\bcp{Generally not very convincing.}
