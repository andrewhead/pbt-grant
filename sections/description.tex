...
One such technique, {\em property-based testing} (PBT), has enjoyed widespread
popularity in the functional programming community since its introduction in
the QuickCheck library~\cite{ClaessenHughes00} in Haskell. \amh{[minor point] I
read this sentence as ``PBT is already quite popular'' which seems in contrast
to our goal of trying to pave the way for PBT to become even more widely used.
Maybe there is a way of conveying that QuickCheck has gained a significant
foothold in the Haskell community without the term ``widespread popularity''?}
PBT can be thought of as a kind of ``formal specification without formal
verification.''  Programmers are invited to characterize the desired behaviors
of their software systems or individual components at a high level, in the form
of abstract models or algebraic laws; their code is then validated against
these specifications by running it against a large number of automatically
...
strong enthusiasm \todo{details...}.
% These users generally appreciate PBT for its power and broad applicability...

(However, what the world has right now is decent bare-bones tools,
which can be used very effectively by people that know what they are
doing and are willing to work a little.  What's lacking is
comprehensive support for PBT, both technically (in people's
programming environments) and culturally/educationally (in what people
are taught about PBT and the information resources / best practices
/etc. they have available).)

We heard about this in the Jane Street study, where users
(a) sometimes struggle to identify
situations where PBT will be valuable \amh{Here I think it is important to have an affirmation that there are indeed cases where PBT will be valuable to them that they just have not yet identified},
...
%
\todo{Contributions}
\amh{One part of the vision that I want to see here is the transformation
we think we could see in who uses PBT, and how often they use it. One of
the worrying aspects of the description above is that by mentioning
``abstract models'' and ``algebraic laws,'' readers might be primed to
think that these tools are only really suitable in a limited number of
problems by a limited number of users, and so the impact of this project
would be limited to making the work easier for that limited number of
problems and users. If we think that updates to tooling and education could
lead to these tools---which might be seen as niche---to broad use across
the software industry by a new contingent of developers, I think we should
say so explicitly.}

(shorten to just a TOC:)
...
(\sectionref{sec:ed}).  
\bcp{mention broader impacts and work plan sections.}
...
%
% Property-based testing is a form of random
% testing~ that was popularized by the
% QuickCheck~\cite{DBLP:conf/icfp/ClaessenH00} library in Haskell.
\amh{I'd propose leading in with a sentence asserting the power of PBT to show why
we should care. For instance, ``Property-based testing tools provide programmers
with an efficient means for testing their software's behavior to level of
comprehensiveness not possible with alternative tools.'' Then we can
drill down into the details of how PBT tools accomplish that aim.}
Property-based testing tools allow users to write executable functions
that act as partial
...
\amh{[proposal] Alternative wording for a reader of the proposal like me, who
would need a little hand-holding for this syntax:
What deserves note is that this single property listing takes only a single
line to express, and yet can be used to validate a limitless number of
input-output pairs. This line defines a property, which is implemented as
a simple function that takes a set of arguments and returns a Boolean value:
true if the property is satisfied, and false if it is not. This particular
function, \texttt{prop\_insertCorrect}, checks that the insert operation on
the binary search tree preserves the binary ordering of the tree.
The function can be read as stating that, given an arbitrary tree ``\texttt{t}'' and an
integer ``\texttt{x}'' to insert into that tree, as long as the tree
is a binary search tree (``\texttt{isBST t}''), then the tree should still be
a binary search tree following the insertion of the next integer
(``\texttt{isBST (insert x t)})''.}
This is an ordinary function that takes as inputs an integer and a
tree and asserts that, if the tree satisfies the BST invariant, then
so does the tree that results from inserting the integer.
...
One approach to input generation, which is the approach used by
the tools we study in this proposal, is {\em random}
...
above section---I am not sure that we need to sell random testing as
...
introduce them alongside novel findings in the ``Motivation'' or ``Foundations'' sections.}
...
\hg{Half page over at the moment}
...
AFL~\cite{afl-readme}\footnote{We plan to use AFL++~\cite{fioraldi_afl_2020},
which has supplanted the original project.}, one of the most well-established
...
developers we talked to complained that understanding generator distributions is
hard; they asked for better tools for understanding what their generator is
doing.  Such tools could help developers avoid mistakes that hamper their
testing efforts and make it easier to quickly adapt and tune their generators to
their specific needs.

Understanding generator distributions is challenging for a few reasons. First,
the values produced by generators are often highly structured, with complex
shapes (think lists, trees, and other algebraic data types). Such values cannot
simply be plotted on a chart. Second, even spot-checking the values produced by
a generator is challenging since the values are often quite large. And finally,
writing generators is already considered by developers to be a burden; any tool
in this space needs to be incredibly automatic and easy to use, or developers
may conclude it is not worth the trouble.
...
With these challenges in mind, we propose an early prototype of \genvis, a tool
that gives developers comprehensive visualizations of their generators'
distributions from directly within their code editor. \genvis{} will address the
challenges with visualizing generator distributions by leveraging program
...
for lists: length; for trees: size, depth, balance factor, etc.). An initial set
of features is extracted from the surrounding programming environment and
validated by the user; this will likely result in a few useful visualizations,
but maybe not enough. To ensure a comprehensive understanding of the data,
\genvis{} will use simple synthesis techniques~\cn{}, to compose and combine the
initial set of features into more interesting features. This new round of features
may show the relationship between first-round features, compose the functions to
perform some kind of reduction in dimension\hg{idk}, or \hg{idk}. All of the
features, original and synthesized, will be made available to users in a variety
of charts and graphs.

Of course, showing a developer dozens of charts for their generator may be
overkill, so \genvis{} will also have a way for users to specify which features
are interesting (and which to hide). We will base these interactions based on
the literature around visualization recommendation, specifically
Voyager~\cite{wongsuphasawat_voyager_2016}.

Finally \todo{live programming}

\todo{Talk about DOT vis and ease of use}
