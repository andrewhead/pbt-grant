...
study~\cite{goldstein2022some}, some users indicated just that.
...
examples.
\proposecut{We propose advances in tools
...
learning about PBT.
\proposecut{Our final aim in this project is to develop curricula,
...
\proposecut{Thus, PBT education should concentrate on
disseminating understanding of
situations where PBT shines, so more developers can get a foot in the door
...
at a major professional developer conference\proposecut{ in the functional programming community}.
...
meaningful property. Extending prior work on property extration techniques (see for
...
\end{ganttchart}
  ...
  \amh{Some suggestions follow}
  \amh{Can we move both ``Generalizing'' and ``Observing'' into Y1--2? As
  foundations for the other projects, it feels strangte to have them start after
  some of the new tool work, espcially properties over logs. I think we also
  want some of these observations to happen before or at least in-tandem with
  the validation interactive systems.}
  \amh{I think ``Evaluating Data Distributions'' should be blue, and ``Tuning
  Data Distributions'' should be gold---if Harry is likely to work on one
  project, it would be ``Evaluating,'' which feels like a prerequisite to
  ``Tuning''}
  \amh{Can we move ``When to Specify It?'' to Y3--4? That way we can have it
  rely on the results of the survey (as we suggest in the text), and set aside
  time for a fuller review of the academic literature and open source projects.
  In that case, we should color the bar yellow.}
...
a number of undergraduates or Master's students), over the course
...
reflective generators and write the {\em When to Specify It!} survey \amh{See
note in figure caption suggesting to defer this survey to later, maybe
post-Harry};
later on, they will develop reflective shrinkers and fuzzers,
launch initial efforts in interface design with the first focus problem of
evaluating data distributions, and advise undergraduates and masters students on
...
and co-advising junior students---e.g.,~\cite{pytestmutagen}. \amh{This sounds
like an unfeasibly large amount of work for a few years for Harry. I think I
would instead propose reflective generators, shrinkers, fuzzers, evaluating data
distributions, and some teaching, and leave it at that.}
...
undergraduate or Master's project centered around turning
