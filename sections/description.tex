...
Reflective generators have myriad uses. \amh{Can we rephrase this to say there
are reasons to think that reflectives would be \emph{useful} in PBT?} First,
%
...
informed by the choices that we saw. Our evaluation\amh{what evaluation? Maybe
needs ref.} shows that reflective
generators approximate the performance of the algorithm
from~\cite{soremekun2020inputs}\amh{not clear why this is important} but work with a larger class of generators.
%
Second,
...
%
The response to reflective generators has been encouraging\amh{whose response?
why does response matter for indicating promise of the research idea?}, but
this paper\amh{what paper? the prior paper?} only scratches the surface when it comes to building and
...
like we have a good shot at doing it here.},
...
should move this idea second (after fuzzers), as it feels less fully developed.}
...
generators into widely-used PBT frameworks.}
...
would benefit from having reflective generators in their PBT frameworks.}
%
...
design a programming discipline that makes the process\amh{what process?} equally safe and
...
reviewers would have the same bias.}
...
to the API\amh{what API? Language?} and test those changes with a new group of users.  The
...
Beyond these measures, we will also look into more automated\amh{what does this
mean?}approaches to
...
Most papers\amh{citations may be expected with this phrasing} in the existing PBT literature use small case studies,
...
to demonstrate the interest\amh{promise?} of an idea, but such evaluations
...
effectiveness \amh{for PBT?}~\cite{shi2023etna}.  By ``easily,'' we mean that we take on the burden of
collecting data and analyzing the results.  The tool\amh{confusing subject when
``tool'' refers to something else later in the sentence}
...
allow the developers of Hypothesis, including Zac Hatfield-Dodds \amh{feels like
random name-drop}, to evaluate their testing tools in a controlled
...
{\em benchmarks for PBT}\amh{Isn't this what Etna already did?}. Similar suites of benchmarks already exist in the
