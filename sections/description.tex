...
%
On GitHub, the Hypothesis library in
Python~\cite{maciver2019hypothesis,HypothesisGithub} has 6.5K
``stars'' indicating interest from developers; Rust
quickcheck~\cite{RustQuickcheckGithub} has 2K, ScalaCheck~\cite{ScalaCheckGithub} has 1.8K,
and Clojure's test.check~\cite{ClojureTest.checkGithub} has 1.1K.
%
By comparison, the latest version of JUnit has 5.6K stars and the
previous version has 8.4K~\cite{JUnitGithub}.  That is, Python's PBT
tool has {\em more stars} than the latest incarnation of the most
popular unit testing framework for Java.  The Hypothesis developers
estimate its user community at around half a
million~\cite{ZacPersonalCommunication}.
%
PBT is now used at companies including Amazon, Stripe, Galois, Quviq, and
IOG (which runs the Cardano blockchain and the Ada cryptocurrency).

\todolater{More transition needed?}
...
also highlighted a range of fundamental {\em usability} challenges.
\bcp{One more sentence outlining the challenges.}
...
Addressing these challenges requires insights from both the
...
methodologies for designing tools---including ``front end'' tools like
data visualizers and user interfaces for IDE plug-ins, naturally, but
also to strike the right balance between expressiveness and
accessibility in ``back end'' components like domain-specific
languages for properties and
generators~\cite{PLIERS,Krishnamiurthi.LTL}\todolater{Refs here in
  comments}.
...
Chasins et al.~\cite{chasins_pl_2021} have argued that a research
...
safe, correct programs. We wholeheartedly agree.  Indeed, the team
