...
values more often is a challenging task, often requiring
...
\bcp{I wonder if we should say something here for readers that are
  wondering ``Isn't coverage-guided fuzzing the answer to all this?''}
...
structure correspond to valid changes.\bcp{??} Alternatively, the developer might change
...
relationships between generated outputs\bcp{inputs generated by the
  generator?  outputs generated from those inputs by the SUT?} and their dependencies on
...
reflective generators\bcp{??}, we will generate inputs in a space ``around'' a
...
setting~\cite{ref:suzuki2017tracediff}; the work of this project would be to
...
includes plans for increasing the {\em diffusion} of PBT into the
software world through both educational materials and real-world tools
that embody our findings.

We begin by describing activities for ensuring that our foundational
