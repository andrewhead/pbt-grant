...
inputs.\bcp{There's a persistent potential confusion through the whole
text around the word ``input''---we mean ``input to the SUT, i.e.,
{\em output} of the generator,'' but this may be hard for readers to
remember, and it leads us into some tangled situations here and there
(I am sometimes puzzled what we mean).  Could
we use a more neutral term like ``test case''?  I really think it
would help readability quite a bit.}
\hg{I always write ``test input'' unless it's totally obvious from context, and
I never use input to refer to anything else. This has been my style guide for a
while, esp in the reflectives work where test inputs are both inputs and
outputs. I think ``test case'' is really only marginally clearer, since many
unit-testing frameworks call each unit test a test case.}
%
...
infrastructure.
\amh{Maybe we should end this paragraph with a statement that ``stars'' are not
a perfect indicator of adoption, though they clearly convey that these tools
resonate with the broader developer community. I myself tend to be skeptical of
motivations that come from GitHub stars :-).}\hg{There's a paper we can cite
that makes a good argument that stars are a useful metric; I'll try to find it
again. In any case, it seems like a shame to interrupt the flow here with too
much waffling, so maybe we can put it in a footnote or just quietly put the citation?}
...
use it or not. But if this is a change that feels worth making I'm fine being overruled.} Like
...
for Penn's introductory computer science course.\amh{This latter goal sounds
random. We should make it make more sense by mentioning that PI Pierce is a lead
for this course; and that this course is the entry point for students who will go on
to develop specialties in PL and HCI.}
...
\proposechange{These techniques tend to work when
programs are short and simple in structure.
While properties can be
rather long, they are often
composed of relatively simple parts (e.g., function
calls plus conjunctions and implications)\bcp{plus equalities, at
  least!}.  For this reason, it may be that these
ideas could generalize to helping newcomers understand those
properties well enough to understand them generally\bcp{``understand those
...
the PBT framework.  \bcp{This seems like a longwinded way of saying,
  ``Lots of properties are probably pretty easy to read even for
  non-experts''...?  I.e., these properties don't need much of an
explanation.}}{Since properties are relatively short programs with predictable
...
\hg{I think this was just written more defensively than it had to
be---properties actually are often pretty short and predictable programs.}
...
\bcp{Yes, I was channeling a grumpy PL/SE reviewer.}
...
environments.\bcp{Meaning the features are old and only the
  way of combining them is novel? }
...
setting~\cite{ref:suzuki2017tracediff}\proposecut{; the work of this project would be to
bring this technology into professional programming environments where traces
for similar inputs are abundant.\bcp{Does that mean that this task is
  primarily engineering?  Or are there conceptual challenges that we
could underscore?}}
...
list is non-empty.\bcp{I don't get this.  Is the regression test
  ...
  behavior after the bug is fixed?  Why would we want the former?}
...
tests\bcp{I don't have a clear picture of what this would involve.
  ...
  responds like that''?  Where are the readability challenges?} (using
...
user needs. Additionally, the existing frameworks have not been designed with
the benefit of the insights and tools that we will build in this project, so
there are potentially huge opportunities for improvement.
Thus, we will allocate engineering time to supporting and improving
open-source PBT frameworks.

We will choose three PBT frameworks or tools and use
insights from our research to boost their usability and adoption. We will start
...
main maintainer of Hypothesis, and a letter of collaboration from Mr.{}
Hatfield-Dodds appears in the supplemental documents. The Hypothesis team is
especially excited about integration ideas like reflective generators
(\sectionref{sec:reflective}) and interaction modes like distribution
visualization (\sectionref{fig:gen-vis}) into the framework.

Beyond Hypothesis, we will seek out at two more projects to advance. These
projects will be chosen based on potential impact: we will prioritize great
projects that have significant room for growth and that would benefit
significantly from well-targeted person-hours. Candidates for this include
OCaml's Crowbar, which is not actively maintained but has great ideas and
low-hanging fruit~\cite{noauthor_shrinking_nodate}, and projects like
jqwik in Java or proptest in Rust, which are embedded in massive language
communities that could benefit from increased PBT. Whichever projects we choose,
we will either take them over if they are unmaintained or work closely with the
...
the Diffusion heading (\sectionref{sec:diffusion})---aim to have
broad impacts.  Here we recapitulate those aims and sketch
