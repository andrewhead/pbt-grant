...
% \hg{No idea if this is good, but I'm leaving it here for now:
% \begin{itemize}[noitemsep]
% \item PBT is great
% \begin{itemize}[noitemsep]
%         \item QC most influential and most cited
%         \item It's been used with great success in industry
%         \item Hypothesis has 500k users
% \end{itemize}
% \item We need powerful tools like PBT because software bugs are expensive
% \begin{itemize}[noitemsep]
...
% \end{itemize}
% \item We need to keep pushing to improve testing infrastructure, and there is plenty of room for PBT to grow
% \begin{itemize}[noitemsep]
%         \item The list of companies using it is impressive, but it is still relatively short: PBT has not yet made it into mainstream SE practice
%         \item 500k hypothesis users is only ~3\% of python users
% \end{itemize}
% \item And we know how to push it farther: significantly improved tool support
% \begin{itemize}[noitemsep]
%         \item We want to build human-centered, theoretically motivated tools that
%         actually make PBT better for users
%         \item This has worked for other high-impact software correctness stories
%   \begin{itemize}[noitemsep]
%     \item JUnit took off because it was built into IDEs (is this true??)
%     \item Interactive theorem proving made a huge change in update of formal verification
%   \end{itemize}
% \end{itemize}
% \item To get there, we need to combine PL and HCI
% \begin{itemize}[noitemsep]
%         \item PL provides the background, foundations, and supporting technologies
%         \item HCI provides
% \begin{itemize}[noitemsep]
%                 \item Principled ways of identifying problems
%                 \item Design methodologies with proven results
%                 \item Evaluation metrics that give much faster feedback than just waiting to see if a tool catches on
% \end{itemize}
%         \item This is part of a growing movement (cite Chasins)
%         \item And the ball is already rolling: the bulk of the projects in this proposal are motivated by HCI-style inquiries into PBT
% \end{itemize}
% \item We propose…
% \end{itemize}
% }
...
a program in the form of executable {\em
...
PBT is used at companies including Amazon, Stripe, Galois, Quviq, and IOG (which
runs the Cardano blockchain and the Ada cryptocurrency).

PBT took the functional programming community by storm following its
...
2000'' for their original paper; it is currently the most cited ICFP paper by
...
at around half a million~\cite{ZacPersonalCommunication}.  On GitHub,
...
%
By comparison, the latest version of JUnit has 5.6K stars,\footnote{The previous
version does have a few more: 8.4K.}~\cite{JUnitGithub}--- which means that
Python's PBT tool has {\em more stars} than the latest incarnation of the
framework that is widely credited with popularizing unit testing as a technique.

One might wonder if this is ``enough'' success for PBT, but there is actually
significant opportunity for its continued growth. The 500K estimate for
Hypothesis users is only 3\% of total Python developers~\cn{}, and the list of
companies using is far from the majority of the software
industry---fundamentally, PBT is still not a standard software engineering
technology. And it is not just possible for PBT to grow, it is {\em important}.
...
year. Accelerating the growth of PBT stands to have a significant impact on the
global cost of software bugs.

Our group set out to understand the highest-leverage ways to grow PBT by
...
need more support from tooling to help them apply it most effectively.
...
methodologies for designing tools. Tools, in this case, includes ``front end'' tools like
data visualizers and user interfaces for IDE plug-ins, naturally, but
also ``back end'' components like domain-specific languages for properties and generators---HCI techniques
help designers 
to strike the right balance between expressiveness and
accessibility in their languages~\cite{PLIERS,Krishnamiurthi.LTL}\todolater{Refs here in
  comments}.
Synergies like this led
Chasins et al.~\cite{chasins_pl_2021} to argue that a research
methodology combining PL and HCI hits a ``sweet spot''\bcplater{sweet
  spot for... what?} where need-finding techniques identify current
pain points and motivate concrete tools that help programmers write
safe, correct programs. \proposecut{We wholeheartedly agree.}\hg{Can we say
something about how PL+HCI is a vibrant, emerging research area?}
...
The team
behind this proposal is uniquely positioned to advance the state of
the art at the intersection of these areas: PI Head recently
...
two published
papers~\cite{goldstein_problems_2022,ShiPlateau}\todolast{Make sure
  the PLATEAU paper is in the bib file.} and to the ongoing
...
We propose an interdisciplinary agenda for research and engineering on
...
need to offer \edutheme{Support and Guidance for Open-Source
  Communities} to help our
work and ideas permeate the PBT world.
\rjmhlater{"Existing tools for PBT are inconsistent and
under-maintained"... which tools are you talking about? Some of the 57
doubtless are. But whether those are the ones where your attention
should be directed is another question. In any case, whether guidance
is what is lacking, as opposed to resources, is open to question. But
fundamentally, "undermaintained" is a description that applies to a
LOT of open source software... but this is a
fundamental problem that this project is not going to fix. You need to
be more precise, and more limited in your goals, to be credible here.
}
\bcplater{+1.  I also don't like the title of the last task much.}
...
\SUBSECTION{Cognitive theory of PBT}{sec:cogtheory}{2}{3}{HCI
...
parser by construction\rjmh{Huh?}; ours will also be monadic, but also {\em reflective}.
...
\iflater\hg{Is this everything? Probably not...} \bcp{But maybe all we
  have space for?}

\bcp{Is there anything
we can do to guard against reviewers noticing we've published an ICFP
paper about some of this?}\fi
...
\SUBSECTION{Mixed-initiative property
  specification}{sec:interactive}{4}{4}{HCI Practice}{PhD 4}{PhD 3}{Pierce}
...
\SUBSECTION{Interactive shrinking and
  debugging}{sec:failures}{4}{4}{HCI Practice}{PhD 3}{PhD 2}{Head}
%
\rjmh{I wasn't really convinced by your section on interactive
shrinking. The shrinking search is often long, involving very many
tests, and I certainly don't want to be involved in each step. You
talk about displaying the valid ways a test case can be shrunk, and
letting the user choose which to apply. The choice is easy: do them
*all*! The problems with shrinking are two-fold. First, many
developers don't write shrinkers at all, which results in
incomprehensible failed tests. But secondly, shrinking stops at a test
case which is too large to understand. At that point, I find myself
inspecting the test for any way in which it might perhaps be
simplified, which the current shrinker hasn't already tried, and then
I want to automate that step too, and rerun the shrinking process. At
present I have to do that by modifying the shrink function,
recompiling the code, and rerunning QuickCheck (or, in the Quviq
version, restarting the shrinking process for the same test case),
hopefully shrinking will proceed better from now on. But it would be
great to be able to make that shrink step, and then continue the
automatic process directly! My point here is that when shrinking
works, there's no need to interact with it, but in more complex cases
it often turns out that custom shrinking steps are needed, so helping
the shrinking with such a step (and then continuing the automated
search) is a place where interaction could be invaluable. Of course,
ideally I want not only to take one custom shrinking step in that
particular case, but synthesize automation that will try similar steps
in every shrinking run in the future too!
%
\\
%
I also noted that shrinking tries many different steps in general, but
we have no way of observing *which steps worked*--and perhaps that
could be used to speed up shrinking automatically. Steps that 'usually
succeed' could be tried earlier, in an autotuning process. That could
be interesting.
}
%
...
\SUBSECTION{Counterexamples as regression
  tests}{sec:counter}{5}{5}{HCI/PL Practice}{PhD 3}{}{Head}
%
...
\todo{Add titles}
\begin{itemize}[noitemsep]
  \item \sectionref{sec:frameworks}: Helping PhD \hg{N} to build and maintain a minimal PBT
  ...
  \hg{N} with maintenance of tools for easier property specification.
\end{itemize}
Most importantly, the research engineer will help to build and maintain
\tyche{}, the IDE for PBT designed as part of PhD \hg{N}'s dissertation. We
describe this effort in more detail in the next section.

\SUBSECTION{Tyche: An IDE for PBT}{sec:ide}{2}{5}{Tech Transfer}{Engineer}{PhD
3}{Pierce}
...
and debugging processes---all without requiring the programmer to context-switch
out of their normal workflow. The research engineer will assist in the
integration process, ensuring that the extension is more than the sum of its
parts. Indeed, we plan for \tyche{} to live on as a home for further innovation:
future tools that we or others in the community see as important improvements to
PBT workflow will also be included over time.

With a strong theoretical grounding and consistent engineering support, \tyche{}
will provide an efficient pipeline for bringing research on PBT interaction
models to software developers craving better tools.
