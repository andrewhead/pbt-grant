...
\amh{I removed the mention of the HATRA workshop paper because I could not think
of how to include it, though we may need to add it back in.}
...
bugs. Accordingly, we propose to develop tools to help developers write
specifications in the context of high leverage scenarios. To begin, we
propose tools
for supporting existing high-leverage scenarios, in particular {\em
model-based} or {\em differential} testing of classes and
modules~(\sectionref{sec:model-based}). Next we propose a technique for
...
to assemble these
techniques and others together into a unified toolkit, {\em Ghostwriter++}, for
automatic property inference.
%
Success in these tasks will again be measured both indirectly, through
uptake of
...
\SUBSECTION{Supporting High-Leverage Scenarios: Model-Based
...
Jane Street,
environment, it is quite common for developers to build (or already have) a {\em
...
{\em differential testing}~\cn.
%
Since this situation is common, we want to make it as easy as possible for
developers.  We have already carried out some early experiments in
{\sc Mica}~\cite{Ng2023MicaPoster}, a tool
that automates the process of writing model-based properties for languages like
OCaml, Standard ML, and Scala~\cite{macqueen_modules_1984}; we propose
to continue this work and broaden it to work in a wider range of languages.

{\sc Mica} automatically builds test suites to compare multiple OCaml
modules implemnenting the same interface. Given a module signature
(\lstinline{S}) and two modules
(\lstinline{M1} and \lstinline{M2}) implementing that signature, {\sc Mica}
...
The current version of {\sc Mica} works with a very limited subset of
the features in the OCaml
module system; many more remain to be addressed. In particular, it
works with purely functional modules with at most one opaque type
member, such as a {\tt Set} module whose type member hides the
representation of sets.  It does
not yet handle modules with muitable internal state,
Moreover, it relies critically on
the module's type signature to assemble
chains of operations\bcp{??}; but we would like to extend this technique to work in more
weakly-typed languages like Python. \hg{And we think these are possible
because...} \bcp{Yeah, why do we think this??}

\bcp{Needs more detail about why what Mica does now is nontrivial and
  (most importantly) about what module system features we want to
  address---not just state (though we need a better explanation of
  that too), but also functors.}

The work in this project will ultimately provide a recipe for
automating a significant swath of module testing situations,
significantly lowering the barrier to entry for PBT.

\publicationtarget{International Conference on Functional Programming (ICFP)}

\SUBSECTION{Expanding High-Leverage Scenarios: Patterns over
Logs}{sec:ltl-logs}{3}{3}{HCI Practice}{REPL}{PhD 4}{Pierce}
%
\hg{Could crunch, but I actually still like this section}
...
the case of P7\iflater\bcp{This might be the first place where we
  mention a particular participant now.  In fact, it might be the ONLY
place that does.  If so, probably best to rewrite.}\fi{} from our user
...
components was too fuzzy.\bcp{``because the abstraction boundary between the
components was too fuzzy'' doesn't make obvious sense to me---why is
this an issue of poor abstraction?}

We will develop a tool that allows developers to express properties that reach
into\bcp{``reach into'' is not very intuitive for me} the state of multiple interacting software components. Take again the
...
\lstinline{never} from the example properties above. This language will
take inspiration from temporal logics such as linear temporal logic (LTL).

Temporal logics have been used for PBT in the
...
\SUBSECTION{Automating High-Leverage Scenarios:
Ghostwriter++}{sec:ghostwriter}{3}{3}{HCI Practice}{PhD 2}{PhD 4}{Head}
%
...
property generation for \hg{...}.  We would like to expand Ghostwriter to
automatically infer properties in a much wider set of scenarios.

Our re-imagining of Ghostwriter, dubbed {\em Ghostwriter++} will be a
platform for all
kinds of property automations. It will include all of the tools in the current
version of the tool, {\sc Mica} and \hg{Name?} from earlier in this section, and
other tools for property inference from the literature like QuickSpec~\cn
\hg{and/or the version from Python?}. Additionally, we will consider more
experimental tools for property inference. Tools like TOGA~\cn{} demonstrate
that large language models may be able to help with unit test synthesis, and we
are eager to see if the same applies to PBT.

\hg{This might be bad}\bcp{Not getting it yet, anyway.}
We will unify these tools through two interfaces. First, a straightforward menu
will allow users to click through and interact with the various tools manually.
This will already be immensely helpful, because it will help developers know
what kinds of properties are available and how they might be able to produce
those properties with the help of specialized tools. Second, we will explore
providing a freeform chat interface, with the help of a large language model
like \hg{IDK}, that will be fine-tuned with information about the available
automations and can help developers think through those automations in the
context of their application.

\hg{Sentence or two to wrap up}

\bcp{I'm not feeling this section yet.  At the moment, it reads like a
proposal to just bundle together a few existing methods---it's not
clear what is being added.} \amh{Agreed that this isn't quite there
yet. It feels like we want to say something either about (1) new kinds
of algorithms for synthesizing specs that go beyond Hypothesis',
QuickSpec's, and Mica's (maybe focusing on some of new classes of
properties found from our study) or (2) something that goes a bit
deeper from an interaction design perspective. We might need to answer
the question ``why not just ask ChatGPT to write properties? why does
there need to be any more interface than that?''}\bcp{I think we might
do well to really address the LLM question head on, both early and
late.}

\publicationtarget{IEEE Visual Languages and Human-Centric Computing
  (VL/HCC) \bcp{and maybe UIST?}}
...
In this section, we
describe a thread of research that aims to design and evaluate novel
{\em modes of interaction} for PBT. The projects in this thread will
contribute new
techniques for assessing whether their generators are generating
sufficient and
appropriate inputs,and
altering their generators to achieve better distributions
(\sectionref{sec:evaluating_distributions}).
...
write concrete test cases by hand,
PBT tools draw test inputs automatically from some
random distribution. The effectiveness of
PBT thus depends on the quality of this distribution---whether
it leads to tests that are both sufficiently
''interesting'' and sufficiently diverse.

The challenge is that it is by no means easy for a developer to determine if a
distribution is a ``good'' one\bcp{this can be refined: there are
  really two connected problems---understanding what the distribution
  {\em is} (calculating probababilities of particular tests from some
  opaque or gnarly description of the generator) and understanding
  whether it is good.  Both are hard}. One common strategy is to inspect a
small number of inputs by hand, but this strategy obviously leaves developers
without an understanding of the breadth of what is tested by the
tests\bcp{Not sure I agree with that.}.
...
~\cite{ref:hohman2020understanding}) and visualizing sequences of
program values (e.g.,~\cite{ref:kang2017omnicode}), there remains no fitting
solution for the PBT case\bcp{what is different about the PBT
  case?}. We aim to remedy that.  \bcp{not certain what this paragraph
is trying to add}
...
\bcp{Need to introduce the overall idea of Tyche before getting to its
specific parts.}
...
\bcp{Merge the following with the above --- indeed, this material
  probably belongs first!}
...
\SUBSECTION{An IDE for PBT}{sec:ide}{2}{3}{Tech
Transfer}{???}{}{Pierce}
\bcp{This subsection feels out of place---we describe Tyche in a lot
  of detail in section 5, and we don't make a clean distinction here
  between ``Tyche the research prototype'' and ``Tyche the open-source
project.''  Maybe we should?  Or maybe fold this into section 5?  (On
reflection, I think we should rename this subsection ``Collaborating
with the Open-Source Community,'' and it should talk about both
Hypothesis and (potential) AWS collaborations on turning Tyche into a
serious open-source effort.}
The projects in \sectionref{sec:val} will explore a wealth of new modes of interaction
for PBT. Their potential impact will be magnified by
...
our collaborators on the Hypothesis team (see our collaboration plan) to
...
\bcp{What about the AWS collaboration?  We should mention, at least,
  that we are in the early stages of setting up a collaboration around
making Tyche work for Rust.}
...
also plan to work with 6\bcplater{right?} undergraduate students via an
...
\sectionref{sec:ide}); these will be published under a permissive
...
interested in the results of this proposal.\bcp{mention AWS}
