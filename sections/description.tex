...
(\participant{20}). But participants also described usability issues
...
from scratch!)}\hg{Fine. I just think it's a shame to leave out one of the most
powerful use-cases of PBT when discussing its advantages. Would it help to
change ``formal properties have already been written'' to ``there is already a
specification, as in an RFC or other reasonably formal document'' and then drop the
QuickChick reference?}\bcp{Sure.} Finally, the
...
%
We next describe our plans to address the challenges relating to
\proptheme{specification} that were discussed in the Motivation
section.
...
\lstinline{Inner}, for example:\bcp{I find 1 and 2 a bit unintuitive.}\hg{Not
sure how to improve them without taking space to explain them. I made a couple
of things more clear, but I agree they're still not obvious}\bcp{We've
got a little space to play with now...}
\begin{lstlisting}
...
\end{lstlisting}
Such properties check that internal variables like \lstinline{processed},
...
its \lstinline{Outer}-facing API.
...
situations where it was previously too difficult to apply. \bcp{I'm
  still not that convinced by the examples...}
...
model-based testing for systems built in languages with strong {\em
  module systems}, like OCaml~\cite{macqueen_modules_1984}.  \bcp{A
  potential objection is that languages with ML-style module systems
  are very rare in industry.  We can blunt this objection by talking
about how our results might transfer to other modularity mechanisms
...
its signature. For example, testing the module \lstinline{StringFns} (in
Figure~\ref{fig:sigs}\bcp{Is this the module or the signature?  Seems
  like we want just the signature.}) requires wiring up the functions in the
...
provide useful context for future work in PBT usability.\bcp{Why?}
...
at a major professional developer conference.
...
co-PIs, one PhD student trained primarily in PL, one trained primarily in HCI, and a number of
...
Our research group is already relatively diverse~\amh{I personally do not feel
capable of
describing my group as relatively diverse---while we have better-than-normal
gender
representation, I am not doing well in representing most other marginalized
groups in computing. I wonder if there is a more humble way of phrasing this}---PI Pierce has one
