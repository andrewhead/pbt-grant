...
might want to
test. We propose to address the first of these issues with a new, more
expressive\bcp{is its advantage that it is MORE expressive, or
  DIFFERENTLY expressive?} property language (\sectionref{sec:outpurprop}) and
the second with mixed-initiative\bcp{define/gloss} interactions (\sectionref{sec:interactive}).
%
...
To create PBT tools that work in
cases where important functionality falls across module
boundaries,\bcp{not clear how this fits with ``does not
easily apply to programs with
poorly encapsulated global state, or with leaky or complex abstraction
boundaries.''}
we will explore how to define properties over
cross-module event logs. Consider
...
\publicationtarget{User Interface\bcp{s?} and Software Technology (UIST)}
...
be by existing tooling.\bcp{Emphasize the foundational problems
  here (and everywhere :-)---this is not just engineering!}
%
With this in mind, we plan to build a tool to automate
model-based testing for systems written in languages with strong {\em
  module systems}, like OCaml~\cite{macqueen_modules_1984}.\bcp{Only
  OCaml?  There are plenty of other languages with strong module
  systems...}
...
requires a randomly generated
integer to know how much to drop\bcp{``know how much to drop''?}, calling \lstinline{split} produces a pair of strings
that can each be arguments to future function calls, etc.\bcp{Could be
clearer.} We will build on prior
...
human in the loop).  \bcp{Too big a jump from that to the next sentence.}
...
transfer. \bcp{Why start with OCaml?}
...
programs are short and simple in structure.  While properties can be
...
understand programs by example---in fact, this is the most common
form of testing at Jane Street~\cite{minsky_testing_nodate}\bcp{and
everyone else, pretty much!}---so explaining properties with examples is a
natural choice. Deciding exactly which examples to show may be tricky,
but prior work
suggests some heuristics~\cite{ref:gerdes2018understanding}.
At the very least, the tool can prioritize simpler inputs via shrinking\bcp{??}.  This
...
{\em interaction} for PBT. These projects will contribute new paradigms\bcp{really?} for (1) assessing whether their generators are generating sufficient and
...
(\sectionref{sec:tuning})\bcp{how is (2) different from (1)?},
...
regression tests (\sectionref{sec:counter}).  \bcp{The last, especially,
sounds like ``Just engineering.''  How do we signal that there is
research content there?}
%
...
comparing developers' effectiveness and efficiency in testing (and in debugging problems discovered by testing).  PI Head will
guide tool development\bcp{again, can we make it sound researchy?},
leveraging his
...
the tool {\em is} the theorem...} for
...
be based on awareness of common features of algebraic data
types\bcp{what is ``awareness of common features of algebraic data
  types''?} and extensible
through lightweight user-provided code. For example, consider the
\lstinline{log} type
...
We expect that a other projects in a variety of languages will be
important to assist in a similar way.
\bcp{I wonder if this is going
  to put people's backs up---might sound like we are offering an
  engineering service to the whole community...}
