...
We discuss plans for work that addresses \proptheme{specification
  themes} in \sectionref{sec:spec}.  \bcp{And the education section!
  I think the discussion above is fine---we just need to put a second pointer
here.}
...
\SUBSECTION{Generalizing the Jane Street Findings}{sec:survey}{1}{1}
...
\SUBSECTION{Observing PBT in Practice}{sec:observations}{2}{2}
...
\bcp{mention the section we moved to education}
...
history of successful projects with Prof. Lampropoulos. \bcp{Citations}
...
\SUBSECTION{Evaluating Data Distributions}{sec:evaluating_distributions}{3}{4}
...
ways to manipulate those objects {\em in situ}.\hg{Did I use
that right?} \amh{lgtm!}
...
\amh{I think you're right, though it is risky to lead in
with DOT graphs first, because it feels to me unlikely to
generalize nicely. As I understand, many generated inputs
will have hundreds or thousands of nodes that simply would
not fit be comprehensible in a DOT graph. (And I'm wrong
about that, then some of the reviewers might be too, and in
that case we should explicitly state that generated outputs,
although complex in structure, are likely to have less than
100 nodes and be conducive to DOT visualization).}
...
\SUBSECTION{Counterexamples as Regression Tests}{sec:counter}{3}{4}
