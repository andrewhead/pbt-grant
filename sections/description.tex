...
students can go on to help bootstrap adoption of the same ideas in industry.  PBT
would be a good fit for a number of standard CS courses---compilers,
...
%
Teaching PBT alongside data structures has good
precedent~\cite{wrenn2021using, nelson2021automated}\bcp{what are we
  going to do that's different from what they did?} and is natural for a number
of reasons.  The course is often taken early, so habits
can carry over to other courses. In
addition, data structures often have ``obvious'' properties that can be a great
place to start with PBT (see \sectionref{sec:whento}).
%
Concretely, we plan to design a model for adding PBT to data structures courses
and implement this model in the University of Pennsylvania's CIS 1210,
Data Structures and Algorithms. We will
add a single lecture early in the course on PBT and modify  the
(Java-based) programming assignments to use PBT alongside or instead
of unit testing.
Materials we produce will be made public so other
universities can copy and adapt them to their own courses. We will also
