...
clear what is being added.} \amh{Agreed that this isn't quite there yet. It feels like we want to say something either about (1) new kinds of algorithms for synthesizing specs that go beyond Hypothesis', QuickSpec's, and Mica's (maybe focusing on some of new classes of properties found from our study) or (2) something that goes a bit deeper from an interaction design perspective. We might need to answer the question ``why not just ask ChatGPT to write properties? why does there need to be any more interface than that?''}
