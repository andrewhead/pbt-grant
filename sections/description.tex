...
(\sectionref{sec:whento}), but if done right this could provide invaluable
feedback to PBT researchers about how far their work could reach.

\SUBSECTION{Observing PBT in Practice}{sec:observations}{2}{3}{Other}
...
Concretely, the tools we build will take a model signature, for
example\bcp{Cut the code}
...
Section \sectionref{sec:whento} incorporates the ideas from this section along
...
A developer can better understand a failure if the input that triggered it is
simple. We propose rapid, incremental, interactive shrinking of complex inputs
into simpler inputs.  Shrinking is a well known technique in the PBT
literature~\cite{hughes_quickcheck_2007,arts_shrinking_2014}, and
\sectionref{sec:shrinking} presents a novel approach that makes shrinking
automatic when a reflective generator is available for a given type. But the
shrinking process is often opaque and difficult to control.  Our novel shrinking
can be done {\em interactively}.

\amh{Add references to the related work of
TraceDiff~\cite{ref:suzuki2017tracediff}
JetBrains object browser interfaces~\cite{tool:jetbrains},
Telescope~\cite{ref:hibschman2016telescope},
Ply~\cite{ref:lim2018ply},
Whyline~\cite{ref:ko2009finding},
Gilligan~\cite{ref:holmes2012systematizing}.}
...
\SUBSECTION{Evaluating Data Distributions}{sec:evaluating_distributions}{1}{2}{Harry}
...
\writeme{Needs introduction.}
\ifdraft
Prior work~\cite{wrenn2021using,nelson2021automated}
\fi

\SUBSECTION{When to Specify It!}{sec:whento}{3}{4}{Harry}
%
PBT is often described as a kind of lightweight formal method.   One
...
study that we carried out in preparation for the larger Jane Street
study~\cite{ref:goldstein2022some} seemed to indicate just
that. But at  
...
\SUBSECTION{Interactive Property Specification}{sec:interactive}{2}{3}{Other}
...
\SUBSECTION{PBT in the Undergrad Curriculum}{sec:1210}{2}{4}{Other}
