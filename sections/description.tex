...
One of the most important---and difficult---tasks in the PBT process is 
designing and implementing the random data generators that used to check that 
each property holds. Indeed, the developers we interviewed consistently cited 
designing generators as one of the hardest parts of the PBT process, and many 
developers asked for better tools for constructing generators that accomplished 
their testing goals. In this section, we describe a series of projects to 
develop new and better ways to express random data generators.
...
the test code; (2) the generated outputs can be complex; (3) a developer does 
not care just about singular outputs, but rather the distribution of generated 
outputs, that it covers a sufficiently interesting and representative space of 
inputs; and (4) as with conventional debugging, it can be difficult to 
understand the cause of software failures---though here PBT provides a unique 
opportunity, because failed inputs might be best understood in terms of 
deviation from behavior for nearby successful inputs. Our goal in the upcoming 
work is to develop a vision of what great testing tools look like for a 
multiplicity of generated inputs. This is an area that has not yet received 
attention in the area of HCI for programming tools, and one that is fruitful 
with exciting areas to explore and with high potential for impact.}
