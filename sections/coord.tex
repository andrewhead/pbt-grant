...
of five years, summarized in Figure~\ref{fig:workplan}.
...
with varying background and interests, one staff engineer, and many
...
\caption{Plan of work.
  ...
  \bcp{The years need to be rethought for all of these!}}
%   \hg{I'm afraid we may have over-indexed on these categories. For one thing,
%   bucketing a project as one thing or another sort of undersells the breadth of
%   some of the projects---there may be interesting theory work in properties over
%   logs or models for modules and interesting practical work in the Explaining
%   Properties thread. It's also not a perfect categorization system anyway, since
%   things like Interactive Property Specification are really both Education and
%   HCI Practice. My suggestion is that we either widen the labels (HCI / PL / Ed)
%   or do away with them entirely and color code by primary owner instead.}
% \bcp{Agreed: The buckets were a good idea, but they are
%   misleading. Let's just make them ``who supervises.''  We could
%   add a column to the textual table if we want.  But maybe it is just
%   getting to be too much information...}
\label{fig:workplan}
\end{figure}
...
\begin{tabular}[c]{lllll}
  Task & Main effort & Supporting effort & Supervisor & Category \\
  \hline
  \input{main.workplan}
\end{tabular}
\caption{Project tasks and personnel \bcp{If we keep the Category
    ...
    categories.}\bcp{Note some ???s.} \bcp{(Remember to) add section
