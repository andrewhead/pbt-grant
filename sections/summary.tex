...
\bcplater{Do we want to try to refocus the summary like we did the
...
What are these challenges? \iflater\hg{I suspect this is where we'll need to come down
stronger---these don't really land as ``grand'' challenges. I think we need to
couch JS and follow-on work as only the beginning}\bcp{I tried a
little rewording (reserving ``challenges'' for bigger stuff and using
``issues'' here), but I didn't like it much.}\fi
...
software foundations or other graduate-level courses for property-based testing?}
%
% \iflater
