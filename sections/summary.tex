...
\amh{We might be at >6 lines of text per inch. I measured 6 and then part of
one. Let's double check with another ruler (one that wasn't printed).}
...
\amh{There may be a chance here to soften the wording to avoid looking
like we are ``overfitting'' on Jane Street}
...
experts in
both areas\iflater \bcp{to... (do something impressive :-)} \amh{Maybe imbue PBT
tools with an unprecedented level of controllability and visibility?}\fi.
...
test-cases for easier debugging \amh{cut this}, mutating inputs to explore program
behavior \amh{cut this, I think}, and tuning distributions based on examples or coverage. It
...
modular abstractions, and techniques for explaining properties \amh{cut this}
\amh{Instead, develop programming assistant interactions that bring together
specification generation techniques and contemporary AI to help programmers
ideate about and implement properties. Except we should say that in half the
space :-)}.
%
...
distributions, simplifying test-cases \amh{cut this}, and saving
  failing test-cases as understandable unit tests \amh{cut this}, culminating in a VSCode
  extension for PBT \amh{maybe this can be stated a little more
  excitingly---like ``a first-of-its-kind interactive IDE for PBT''}.  \iflater\bcp{make sure all that is accurate}\fi
...
industry leaders and through a broad \amh{maybe cut ``broad'', maybe not broad in the new
3-year framing} range of educational activities.
...
themes described above. \amh{Cut this} \amh{Instead, develop a base IDE for PBT
and promote its adoption and evolution in partnership with our industry
collaborators.}
We will also
...
and put it on every
software developer's testing toolbelt. \amh{Here is an opportunity to restate
the ``something impressive'' from the overview.} Better testing, in turn, will
